...
function validateAPINames(obj, type, className) {
	...
	if (obj.length > 0) return "Could not find: " + obj;
}
...
function validateConstants(constants) {
	...
	if (Array.isArray(constants)) {
		...
		if (!(cls in doc) || !("properties" in doc[cls]) || doc[cls] == null) {
			...
			if (prop.charAt(prop.length - 1) == '*') prop = prop.substring(0, prop.length - 1);
		...
		if (~common.VALID_OSES.indexOf(key)) {
			for (var x in oses[key]) {
				...
				switch (x) {
					case "max" :
					case "min" :
						if (err = validateVersion(oses[key][x])) errors.push(err);
						break;
					case "versions":
                        oses[key][x].forEach(function (elem) {
							if (err = validateVersion(elem)) errors.push(err);
						});
						break;
					default:
...
function validateReturns(ret) {
	...
	if (Array.isArray(ret)) {
		...
		for (var key in ret) {
			switch (key) {
				case "type" :
					if ((err = validateDataType(ret[key])) && ret[key] != 'void') errors.push(err);
					break;
				case "summary" :
					if (err = validateString(ret[key])) errors.push(err);
					break;
				case "constants" :
					if (err = validateConstants(ret[key])) errors.push(err);
					break;
				default :
...
function validateKey(obj, syntax, currentKey, className) {
	...
	if (Array.isArray(syntax)) {
		if (syntax.length == 1) {
			if (Array.isArray(syntax[0])) {
                ...
                obj.forEach(function (elem) {
					...
					for (var key in elem) {
						if (key in syntax[0]) {
							if (err = validateKey(elem[key], syntax[0][key], key, className)) errs[key] = err;
						} else {
		...
		for (var key in obj) {
			if (key in syntax) {
				if (err = validateKey(obj[key], syntax[key], key, className)) errors[key] = err;
		...
		switch (syntax) {
			case "Boolean" :
				if (err = validateBoolean(obj)) errors[currentKey] = err;
				break;
			case "Class" :
				if (err = validateClass(obj)) errors[currentKey] = err;
				break;
			case "Constants" :
				if (err = validateConstants(obj)) errors[currentKey] = err;
				break;
			case "DataType" :
				if (err = validateDataType(obj)) errors[currentKey] = err;
				break;
			case "Default" :
				if (err = validateDefault(obj)) errors[currentKey] = err;
				break;
			case "Number" :
				if (err = validateNumber(obj)) errors[currentKey] = err;
				break;
			case "OSVersions" :
				if (err = validateOSVersions(obj)) errors[currentKey] = err;
				break;
			case "Primitive" :
				if (err = validatePrimitive(obj)) errors[currentKey] = err;
				break;
			case "Returns" :
				if (err = validateReturns(obj)) errors[currentKey] = err;
				break;
			case "Since" :
				if (err = validateSince(obj)) errors[currentKey] = err;
				break;
			case "String" :
				if (err = validateString(obj)) errors[currentKey] = err;
				break;
			default:
				if (syntax.indexOf("Array") == 0) {
					switch(syntax.slice(syntax.indexOf('<') + 1, syntax.indexOf('>'))) {
						case "events.name" :
							if (err = validateAPINames(obj, "events", className)) errors[currentKey] = err;
							break;
						case "methods.name" :
							if (err = validateAPINames(obj, "methods", className)) errors[currentKey] = err;
							break;
						case "properties.name" :
							if (err = validateAPINames(obj, "properties", className)) errors[currentKey] = err;
							break;
						default :
