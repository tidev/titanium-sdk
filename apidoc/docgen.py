...
class API(object):
	...
	def set_subtype(self,typestr):
		...
		if self.subtype == 'view':
			...
			self.add_property('anchorPoint','object','a dictionary with properties x and y to indicate the anchor point value. anchor specifies the position by which animation should occur. center is 0.5, 0.5, ie anchorPoint:{x:0.5,y:0.5}')
			self.add_property('transform','object','the transformation matrix to apply to the view')
			self.add_property('center','object','a dictionary with properties x and y to indicate the center of the views position relative to the parent view, ie center:{x:0.5,y:0.5}')
			...
			self.add_property('size','object','the size of the view as a dictionary of width and height properties, ie size:{width:100,height:100}')
