...
template_dir = os.path.abspath(os.path.join(this_dir, "..", "templates", "modulehtml"))
...
# The base url to be use for objects that are linked to but are not part of the current module
appc_docs_base_url = "http://docs.appcelerator.com/titanium/latest/#!/api/"
...
def generate(raw_apis, annotated_apis, options):
	...
	log.info("Generating Module HTML")
	...
	if options is not None:
		...
		for name in annotated_apis:
			...
			if one_type.external:
				continue
			log.trace("Producing html output for %s" % name)
			render_template(one_type, options)

		# Generate an index.html, it will redirect to the main module object
		...
		index_file.close()
		log.info("An index.html file has been written to %s and will redirect to the main module object." % os.path.abspath(options.output))
		...
		# Copy css file from template folder to dist
		if options.css is not None:
			css_source_path = os.path.abspath(os.path.join(template_dir, "%s" % options.css))
			if os.path.exists(css_source_path):
				css_dest_path = os.path.abspath(os.path.join(options.output, "%s" % options.css))
					...
					if hasattr(obj, list_name) and type(getattr(obj, list_name)) == list:
						for m in getattr(obj, list_name):
							if m.name == member_name:
								...
								# MODULES 
								# Links to "methods", "properties", "events" will point to their parent object
								if parent.startswith("Modules."):
									m = obj
									fragment = "#%s" % member_name
