 ...
 */
export class AppleWatchCreator extends Creator {
	/**
	...
	configOptionProjectDir(order) {
		...
		function validate(projectDir, callback) {
			...
			while (!fs.existsSync(file)) {
				...
				if (dir === root) {
					if (projectDir !== '.') {
						logger.error(`Invalid project directory "${projectDir}" because the tiapp.xml is not found`);
					}
					return callback(true);
				}
				file = path.join(dir, 'tiapp.xml');
	...
	configOptionAppName(order) {
		...
		return {
			abbr: 'n',
			desc: 'the name of the watch app',
			order: order,
			prompt: function (callback) {
				callback(fields.text({
					promptLabel: 'Watch app name',
