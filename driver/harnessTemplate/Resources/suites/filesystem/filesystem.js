module.exports = new function() {
	...
	this.appendStringTest = function() {
	...
	this.fileStreamBasicTest = function() {
	...
	this.fileStreamWriteTest = function() {
	...
	this.fileStreamAppendTest = function() {
		var infile = Ti.Filesystem.getFile(Ti.Filesystem.resourcesDirectory, 'suites/filesystem/stream_test_in.txt');
		var instream = infile.open(Ti.Filesystem.MODE_READ);
	...
	this.fileStreamPumpTest = function() {
	...
	this.fileStreamWriteStreamTest = function() {
	...
	this.fileStreamResourceFileTest = function() {
		if(Ti.Platform.osname === 'android') {
			valueOf(function() {Ti.Filesystem.openStream(Ti.Filesystem.MODE_WRITE, Ti.Filesystem.resourcesDirectory, 'suites/filesystem/stream_test_in.txt')}).shouldThrowException();
			valueOf(function() {Ti.Filesystem.openStream(Ti.Filesystem.MODE_APPEND, Ti.Filesystem.resourcesDirectory, 'suites/filesystem/stream_test_in.txt')}).shouldThrowException();
	...
	this.fileStreamTruncateTest = function() {
		var inBuffer = Ti.createBuffer({value:"huray for data, lets have a party for data1 huray for data, lets have a party for data2 huray for data, lets have a party for data3"});
		valueOf(inBuffer).shouldBeObject();
		var inStream = Ti.Stream.createStream({source:inBuffer, mode:Ti.Stream.MODE_READ});
		valueOf(inStream).shouldNotBeNull();
	...
	this.filesInApplicationCacheDirectoryExists = function() {
		if(Ti.Platform.osname === 'android') {
