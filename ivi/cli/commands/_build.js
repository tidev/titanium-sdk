/*
 * build.js: Titanium Mobile IVI CLI build command
 *
...
exports.config = function (logger, config, cli) {
	return function (finished) {
		cli.createHook('build.ivi.config', function (callback) {
			callback({
				options: {
					'deploy-type': {
						abbr: 'D',
	...
	cli.fireHook('build.pre.construct', function () {
		new build(logger, config, cli, function (err) {
			cli.fireHook('build.post.compile', this, function (e) {
				...
				cli.addAnalyticsEvent('ivi.build.' + cli.argv['deploy-type'], {
...
function build(logger, config, cli, finished) {
	...
	this.buildDir = this.projectDir + '/build/ivi';
...
build.prototype = {
	...
	readTiPackageJson: function () {
		this.logger.info(__('Reading Titanium Mobile Denso IVI package.json file'));
		var mwPackageFile = this.mobilewebSdkPath + '/titanium/package.json';
		afs.exists(mwPackageFile) || badInstall(__('Unable to find Titanium Mobile Denso IVI package.json file'));
		try {
			return JSON.parse(fs.readFileSync(mwPackageFile));
		} catch (e) {
			badInstall(__("Unable to parse Titanium Mobile Denso IVI's package.json file"));
		}
	},
	...
	copyFiles: function (callback) {
		...
		if (afs.exists(this.projectResDir, 'ivi')) {
			afs.copyDirSyncRecursive(this.projectResDir + '/ivi', this.buildDir, { preserve: true, logger: this.logger.debug, rootIgnore: ['apple_startup_images', 'splash'] });
			['Default.jpg', 'Default-Portrait.jpg', 'Default-Landscape.jpg'].forEach(function (file) {
				file = this.projectResDir + '/ivi/apple_startup_images/' + file;
				if (afs.exists(file)) {
	...
	findTiModules: function (callback) {
		...
		appc.timodule.find(this.tiapp.modules, 'ivi', this.deployType, this.titaniumSdkVersion, this.moduleSearchPaths, this.logger, function (modules) {
	...
	assembleTitaniumCSS: function (callback) {
		...
		if (this.tiapp.mobileweb.splash.enabled) {
			var splashDir = this.projectResDir + '/ivi/splash',
	...
	createIcons: function (callback) {
		...
		if (!/\.(png|jpg|gif)$/.test(file) || !afs.exists(file)) {
			file = path.join(this.projectResDir, 'ivi', 'appicon.png');
		}
