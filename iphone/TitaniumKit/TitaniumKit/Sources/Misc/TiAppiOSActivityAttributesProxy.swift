...
public class TiAppiOSActivityAttributesProxy : TiProxy {
  ...
  func startActivity(args: [Any]) {
    ...
    let attributes = TiAttributesSample()
    let contentState = TiAttributesSample.Status(value: params)

    do {
      let _ = try Activity<TiAttributesSample>.request(attributes: attributes, contentState: contentState)
    } catch (let error) {
