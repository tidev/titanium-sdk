...
CGSize SizeConstraintViewWithSizeAddingResizing(LayoutConstraint *constraint, NSObject<LayoutAutosizing> *autoSizer, CGSize referenceSize, UIViewAutoresizing *resultResizing)
{
  ...
  // Apply "minWidth"/"maxWidth" before deriving the height. Content that wraps (labels, vertical layouts)
  // must be measured against the clamped width, otherwise a SIZE view capped by "maxWidth" keeps the
  // height of its unclamped, single-line width and its children get clipped.
  ...
  // The same clamp applies the "maxHeight" ceiling, which is unconstrained by default.
