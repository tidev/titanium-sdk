/*
 * device-installer.js: Titanium iOS device install service
 *
...
export function installOnDevice({ logger, cli, builder, finished }) {
		...
		async.eachSeries(udids, function (udid, next) {
			...
			const handle = handles[udid] = ioslib.device
					...
					if (!handles[udid].logStarted) {
						if (msg.indexOf('{') === 0) {
							try {
								...
								// swallow JSON parse errors from non-header log lines
							}
							skipLine = true;
						}
				...
				.on('error', function (error) {
					...
					let details;
					if (errMessage.indexOf('0xe8008017') !== -1) {
					...
					} else if (errMessage.indexOf('0xe800001a') !== -1) {
					...
					next(new appc.exception(errMessage, details));
				});
		}, finished);
