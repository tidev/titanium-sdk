...
FRAMEWORK_SEARCH_PATHS = $(inherited) "$(TITANIUM_SDK)/iphone/Frameworks/**"`);
	...
	async buildModule() {
		...
		await new Promise((resolve, reject) => xcodebuildHook(xcBuild, xcodeBuildArgumentsForTarget('iphonesimulator'), opts, 'xcode-sim', (result) => {
		...
		await new Promise((resolve, reject) => xcodebuildHook(xcBuild, xcodeBuildArgumentsForTarget('iphoneos'), opts, 'xcode-dist', (result) => {
			if (result instanceof Error) {
				reject(result);
			} else {
				resolve();
		...
		if (osVersionParts[0] > 10 || (osVersionParts[0] === 10 && osVersionParts[1] >= 15)) {
			...
			if (this.isMacOSEnabled) {
				...
				console.log('Creating build for mac-catalyst');
				await new Promise((resolve, reject) => xcodebuildHook(xcBuild, xcodeBuildArgumentsForTarget('macosx'), opts, 'xcode-macos', (result) => {
					if (result instanceof Error) {
						reject(result);
					} else {
						resolve();
