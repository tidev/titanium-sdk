...
-(void)setInnerView:(UIView *)innerView
{
    ...
    if ([_innerView isKindOfClass:[UIToolbar class]]) {
        _isToolbar = YES;
    }
...
-(void)layoutChildren
{
    ...
    for (NSUInteger index = startAt, length = [subviews count]; index < length; index++)
    {
        TiLayoutView* child = [subviews objectAtIndex:index];
        if (![child isKindOfClass:[TiLayoutView class]]) {
            NSLog(@"[DEBUG] %d %s Not a TiLayoutView subview: %@", __LINE__, __PRETTY_FUNCTION__, [[child class] description]);
            continue;
        };
...
//        [superview addConstraints: TI_CONSTR(@"H:[superview(self)]", TI_VIEWS(self, superview))];
    ...
    if (![self isInToolbar]) {
        ...
        if (IS_UNDEFINED(right)) {
            [superview removeConstraints:TI_CONSTR( @"H:[self]-(0)-|", TI_VIEWS(self))];
        }
        if (IS_UNDEFINED(top)) {
            [superview removeConstraints:TI_CONSTR( @"V:|-(0)-[self]", TI_VIEWS(self))];
        }
        if (IS_UNDEFINED(bottom)) {
            [superview removeConstraints:TI_CONSTR( @"V:[self]-(0)-|", TI_VIEWS(self))];
        }
    }
    if (IS_DIP(width)) {
