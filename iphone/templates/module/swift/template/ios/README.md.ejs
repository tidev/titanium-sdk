...
Use the `spm.json` file (next to your module `manifest`) to list any Swift Package Manager dependencies. Set `"linkage": "embedded"` to fold the package into your module’s `.xcframework`, or `"linkage": "host"` to have the Titanium CLI add the package to the consuming app so multiple modules can share it.
...
After editing `spm.json`, rebuild the module so the updated Swift package metadata is embedded in the distribution zip.
