...
Declare Swift Package Manager dependencies in the `spm.json` file that sits next to your module `manifest`. Each entry can control whether the package is baked into the module binary (`"linkage": "embedded"`, the default) or added to the consuming app (`"linkage": "host"`). Embedded dependencies keep your module self-contained, while host dependencies let multiple modules share the same package once at the app level.
...
Rebuild the module after editing `spm.json` so the metadata ends up inside the packaged zip.
