...
/**
 * Some modifications to this file are provided by Appcelerator Titanium Mobile
 ...
 *
 * Setting the 'autoaccept' parameter to NO causes the listening socket to not immediately
 * start the data source listeners, and will disable them after they accept a single connection.
 * Note that for interfaces which are accepting both IPv4 and IPv6, this could lead to starvation
 * (one is not preferred over the other and the scheduling of which data source is queried is
 * up to GCD).
 *
**/
- (BOOL)acceptOnInterface:(NSString *)interface port:(uint16_t)port autoaccept:(BOOL)autoaccept error:(NSError **)errPtr;
