...
- (BOOL)addRunLoopMode:(NSString *)runLoopMode
{
  ...
  if (theReadTimer)
    [self runLoopAddTimer:theReadTimer mode:runLoopMode];
  if (theWriteTimer)
    [self runLoopAddTimer:theWriteTimer mode:runLoopMode];

  if (theSource4)
    [self runLoopAddSource:theSource4 mode:runLoopMode];
  if (theSource6)
    [self runLoopAddSource:theSource6 mode:runLoopMode];
...
- (BOOL)removeRunLoopMode:(NSString *)runLoopMode
{
  ...
  if (theReadTimer)
    [self runLoopRemoveTimer:theReadTimer mode:runLoopMode];
  if (theWriteTimer)
    [self runLoopRemoveTimer:theWriteTimer mode:runLoopMode];

  if (theSource4)
    [self runLoopRemoveSource:theSource4 mode:runLoopMode];
  if (theSource6)
    [self runLoopRemoveSource:theSource6 mode:runLoopMode];
...
- (BOOL)acceptOnInterface:(NSString *)interface port:(UInt16)port autoaccept:(BOOL)autoaccept error:(NSError **)errPtr
{
  ...
  if (address4) {
    theSocket4 = [self newAcceptSocketForAddress:address4 autoaccept:autoaccept error:errPtr];
    if (theSocket4 == NULL)
      goto Failed;
  }
...
- (CFSocketRef)newAcceptSocketForAddress:(NSData *)addr autoaccept:(BOOL)autoaccept error:(NSError **)errPtr
{
  ...
  CFSocketRef theSocket = CFSocketCreate(kCFAllocatorDefault,
      ...
      &theContext);
  if (!autoaccept) {
    CFSocketDisableCallBacks(theSocket, kCFSocketAcceptCallBack);
  }
...
- (void)doSocketOpen:(CFSocketRef)sock withCFSocketError:(CFSocketError)socketError
{
  ...
  if (pass && ![self createStreamsFromNative:nativeSocket error:&err])
    pass = NO;
  if (pass && ![self attachStreamsToRunLoop:nil error:&err])
    pass = NO;
  if (pass && ![self openStreamsAndReturnError:&err])
    pass = NO;
...
- (BOOL)attachStreamsToRunLoop:(NSRunLoop *)runLoop error:(NSError **)errPtr
{
  ...
  CFOptionFlags readStreamEvents = kCFStreamEventHasBytesAvailable | kCFStreamEventErrorOccurred | kCFStreamEventEndEncountered | kCFStreamEventOpenCompleted;
...
#pragma unused(timer)
  ...
  if ([theDelegate respondsToSelector:@selector(onSocket:shouldTimeoutReadWithTag:elapsed:bytesDone:)]) {
    timeoutExtension = [theDelegate onSocket:self
                    shouldTimeoutReadWithTag:theCurrentRead->tag
...
- (void)completeCurrentWrite
{
  ...
  if (theCurrentWrite != nil)
    [self endCurrentWrite]; // Caller may have disconnected.
}
...
#pragma unused(timer)
  ...
  if ([theDelegate respondsToSelector:@selector(onSocket:shouldTimeoutWriteWithTag:elapsed:bytesDone:)]) {
    timeoutExtension = [theDelegate onSocket:self
                   shouldTimeoutWriteWithTag:theCurrentWrite->tag
...
#pragma unused(address)
  ...
  case kCFSocketReadCallBack:
    if ([theDelegate respondsToSelector:@selector(onSocketHasConnectionToAccept:)]) {
      [theDelegate onSocketHasConnectionToAccept:self];
    }
    break;
  default:
