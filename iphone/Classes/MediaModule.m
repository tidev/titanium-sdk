    ...
    if ([image isKindOfClass:[TiBlob class]])
    {
        ...
        else
        {
            ...
            if (errorCallback != nil) {
                NSMutableDictionary * event = [TiUtils dictionaryWithCode:-1 message:[NSString stringWithFormat:@"Invalid mime type: Expected either image/* or image/*, was: %@",mime]];
                [self dispatchCallback:[NSArray arrayWithObjects:@"error",event,errorCallback,nil]];
            } else {
                [self throwException:@"Invalid meme type"
                           subreason:[NSString stringWithFormat:@"Invalid mime type: Expected either image/* or image/*, was: %@",mime]
