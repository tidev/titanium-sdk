...
- (void)openUrl:(NSString *)url
  ...
  if ([self isSessionValid] && ([params objectForKey:@"access_token"] == nil)) {
...
- (void)authorizeWithFBAppAuth:(BOOL)tryFBAppAuth
  ...
  if ([device respondsToSelector:@selector(isMultitaskingSupported)] && [device isMultitaskingSupported] && appSupportsBackgrounding) {
    ...
    if (trySafariAuth && !didOpenOtherApp) {
      ...
      NSString *fbAppUrl = [FBRequest2 serializeURL:kOAuthURL params:params];
      didOpenOtherApp = [[UIApplication sharedApplication] openURL:[NSURL URLWithString:fbAppUrl]];
    }
  }
  ...
  if (!didOpenOtherApp) {
    [_loginDialog release];
    _loginDialog = [[FBLoginDialog2 alloc] initWithURL:kOAuthURL
                                          loginParams:params
                                             delegate:self];
...
- (BOOL)handleOpenURL:(NSURL *)url {
  // If the URL doesn't match the structure used for Facebook authorization, abort.
...
- (void)dialog:(NSString *)action
  ...
  if (action == kLogin) {
    ...
    [_fbDialog release];
    _fbDialog = [[FBLoginDialog2 alloc] initWithURL:kOAuthURL loginParams:params delegate:self];
    ...
    [_fbDialog release];
    _fbDialog = [[FBDialog2 alloc] initWithURL:dialogURL
                                       params:params
                                     delegate:delegate];
    [dialogURL release];
