...
- (NSString *)stringFromXMLString:(const xmlChar *)chars {
	...
	if (xmlNode_ != NULL && (xmlNode_->type == XML_ELEMENT_NODE || xmlNode_->type == XML_ATTRIBUTE_NODE || xmlNode_->type == XML_TEXT_NODE)) {
	...
	if (xmlNode_ != NULL) {
		...
		switch (nodeType) {
			case XML_ELEMENT_NODE:
				return GDataXMLElementKind;
			case XML_ATTRIBUTE_NODE:
				return GDataXMLAttributeKind;
			case XML_TEXT_NODE:
				return GDataXMLTextKind;
			case XML_CDATA_SECTION_NODE:
				return GDataXMLTextKind;
			case XML_ENTITY_REF_NODE:
				return GDataXMLEntityDeclarationKind;
			case XML_ENTITY_NODE:
				return GDataXMLEntityDeclarationKind;
			case XML_PI_NODE:
				return GDataXMLProcessingInstructionKind;
			case XML_COMMENT_NODE:
				return GDataXMLCommentKind;
			case XML_DOCUMENT_NODE:
				return GDataXMLDocumentKind;
			case XML_DOCUMENT_TYPE_NODE:
				return GDataXMLDocumentKind;
			case XML_DOCUMENT_FRAG_NODE:
				return GDataXMLDocumentKind;
			case XML_NOTATION_NODE:
				return GDataXMLNotationDeclarationKind;
			case XML_HTML_DOCUMENT_NODE:
				return GDataXMLDocumentKind;
			case XML_DTD_NODE:
				return GDataXMLDTDKind;
			case XML_ELEMENT_DECL:
				return GDataXMLElementDeclarationKind;
			case XML_ATTRIBUTE_DECL:
				return GDataXMLAttributeDeclarationKind;
			case XML_ENTITY_DECL:
				return GDataXMLEntityDeclarationKind;
			case XML_NAMESPACE_DECL:
				return GDataXMLNamespaceKind;
	...
	if (xmlNode_ != NULL && xmlNode_->children != NULL) {
		...
		for (GDataXMLNode *child in children) {
			...
			if (currChildPtr->type == XML_ELEMENT_NODE) {
				...
				// seeking
				if (currChildPtr->ns == childLocalNS && currChildPtr->name != NULL && xmlStrEqual(currChildPtr->name, childDesiredLocalName)) {
					if (array == nil) {
						array = [NSMutableArray arrayWithObject:child];
					} else {
						[array addObject:child];
	...
	if (nodeToFix->type == XML_ELEMENT_NODE) {
		...
		while (definedNS != NULL) {
			...
			xmlNsPtr foundNS = xmlSearchNsByHref(graftPointNode->doc, graftPointNode,
			                                     definedNS->href);
			if (foundNS != NULL && foundNS != definedNS && xmlStrEqual(definedNS->prefix, foundNS->prefix)) {
...
+ (void)fixUpNamespacesForNode:(xmlNodePtr)nodeToFix
	...
	if (nodeToFix->type != XML_ELEMENT_NODE && nodeToFix->type != XML_ATTRIBUTE_NODE)
		return;
