...
- (TiModule *)loadCoreModule:(NSString *)path withContext:(KrollContext *)kroll
{
    ...
    // If there is a core module that collides with the given path,
    // throw an error log
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:[jsPath absoluteString]]) {
		NSLog(@"[ERROR] The requested path %@ collides with the native Ti.%@ component.", path, [path capitalizedString]);
