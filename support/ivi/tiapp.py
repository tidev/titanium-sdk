				...
				if child.nodeName == 'modules':
					...
					for node in child.childNodes:
						if node.nodeType == 1:
							...
							elif node.nodeName in self['ivi']:
								if isinstance(self['ivi'][node.nodeName], dict):
									for subnode in node.childNodes:
										if subnode.nodeType == 1:
											if subnode.nodeName in self['ivi'][node.nodeName]:
												if isinstance(self['ivi'][node.nodeName][subnode.nodeName], dict):
													for subsubnode in subnode.childNodes:
														if subsubnode.nodeType == 1:
															self['ivi'][node.nodeName][subnode.nodeName][subsubnode.nodeName] = getText(subsubnode.childNodes)
												elif isinstance(self['ivi'][node.nodeName][subnode.nodeName], list):
													self['ivi'][node.nodeName][subnode.nodeName].append(getText(subnode.childNodes))
												else:
													self['ivi'][node.nodeName][subnode.nodeName] = getText(subnode.childNodes)
											else:
												self['ivi'][node.nodeName][subnode.nodeName] = getText(subnode.childNodes)
								elif isinstance(self['ivi'][node.nodeName], list):
									self['ivi'][node.nodeName].append(getText(node.childNodes))
								else:
									self['ivi'][node.nodeName] = getText(node.childNodes)
							else:
								self['ivi'][node.nodeName] = getText(node.childNodes)
