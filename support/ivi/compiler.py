...
class Compiler(object):
	...
	def __init__(self, project_path, deploytype):
		...
		self.build_path = os.path.join(project_path, 'build', 'ivi')
		...
		print '[INFO] Titanium Mobile Denso IVI Compiler v%s' % sdk_version
		...
		print '[INFO] Compiling Mobile Denso IVI project "%s" [%s]' % (tiapp_xml['name'], deploytype)
		...
		self.copy(os.path.join(self.resources_path, 'ivi'), self.build_path, ['apple_startup_images', 'splash'])
		self.copy(os.path.join(self.resources_path, 'ivi', 'apple_startup_images', 'Default.jpg'), self.build_path)
		self.copy(os.path.join(self.resources_path, 'ivi', 'apple_startup_images', 'Default-Portrait.jpg'), self.build_path)
		self.copy(os.path.join(self.resources_path, 'ivi', 'apple_startup_images', 'Default-Landscape.jpg'), self.build_path)
		...
		if len(tiapp_xml['modules']):
			...
			for module in tiapp_xml['modules']:
				if module['platform'] in ['', 'ivi', 'commonjs']:
					is_commonjs = False
					
					if 'version' in module and module['version']:
						# search <project dir>/modules/ivi/<module>/<version>/
						module_dir = os.path.join(self.project_path, 'modules', 'ivi', module['id'], module['version'])
						if not os.path.exists(module_dir):
							...
							if os.path.exists(module_dir):
								...
								module_dir = os.path.join(self.modules_path, 'ivi', module['id'], module['version'])
								if not os.path.exists(module_dir):
						...
						# search <project dir>/modules/ivi/<module>/
						module_dir = self.locate_module(os.path.join(self.project_path, 'modules', 'ivi', module['id']))
						if module_dir is None:
							...
							if module_dir is not None:
								...
								# search <global module dir>/<module>/<version>/
								module_dir = self.locate_module(os.path.join(self.modules_path, 'ivi', module['id']))
								if module_dir is None:
									# search <global commonjs dir>/<module>/<version>/
		...
		enableInstrumentation = tiapp_xml['ivi']['instrumentation'] == 'true' if 'instrumentation' in tiapp_xml['ivi'] else False
		ti_js.write(AppcTemplate(codecs.open(os.path.join(self.sdk_src_path, 'config.js'), 'r', 'utf-8').read(), input_encoding='utf-8', output_encoding='utf-8').render(
			...
			ti_fs_registry        = tiapp_xml['ivi']['filesystem']['registry'],
			ti_theme              = theme,
			ti_githash            = self.package_json['titanium']['githash'],
			ti_timestamp          = self.package_json['titanium']['timestamp'],
			ti_version            = sdk_version,
			has_analytics_use_xhr = tiapp_xml['ivi']['analytics']['use-xhr'],
			has_show_errors       = 'false' if deploytype == 'production' or tiapp_xml['ivi']['disable-error-screen'] == 'true' else 'true',
		...
		addProp('ti.fs.backend', tiapp_xml['ivi']['filesystem']['backend'])
		addProp('ti.map.backend', tiapp_xml['ivi']['map']['backend'])
		addProp('ti.map.apikey', tiapp_xml['ivi']['map']['apikey'])
		s = ''
		...
		splash_html = ''
		splash_css = ''
		if tiapp_xml['ivi']['splash']['enabled'] == 'true':
			print '[INFO] Processing splash screen...'
			splash_path = os.path.join(self.project_path, 'Resources', 'ivi', 'splash')
			...
			if os.path.exists(splash_css_file):
				...
				if tiapp_xml['ivi']['splash']['inline-css-images'] == 'true':
		...
		if os.path.exists(icon_file) and (ext == '.png' or ext == '.jpg' or ext == '.gif'):
			...
			icon_file = os.path.join(self.resources_path, 'ivi', 'appicon.png')
		...
		if tiapp_xml['ivi']['filesystem']['registry'] == 'preload':
