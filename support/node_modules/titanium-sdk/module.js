	...
	function searchModuleDir(moduleName, modulePath, platform) {
		afs.visitDirsSync(modulePath, function (version, versionPath) {
			...
			if (afs.exists(manifestFile)) {
				...
				logger && logger.debug(__('Detected module for %s: %s %s @ %s', Object.keys(details.platforms).join(','), details.manifest.moduleid, details.manifest.version, details.modulePath));
			}
		});
	}
		...
		return function(cb) {
			afs.exists(titaniumRoot, function (exists) {
				...
				async.parallel(tasks, function () {
					...
					logger && logger.debug(__('Detecting modules in %s', moduleRoot));
...
exports.find = function (modules, platforms, projectDir, logger, callback) {
	...
	exports.detect(projectDir, logger, function (installed) {
		modules.forEach(function (module) {
			logger && logger.debug(__('Looking for Titanium Module id: %s, version: %s, platform: %s', module.id, module.version, module.platform));
