...
EmulatorManager.prototype.isRunning = function isRunning(name, opts, callback) {
	...
	this.detect(opts, function (err, emus) {
		...
		adb.devices(function (err, devices) {
			...
			require(path.join(__dirname, 'emulators', emu.type + '.js')).isRunning(emu, devices, callback);
		});
	}.bind(this));
};
		...
		if (err) {
			...
			this.detect(opts, function (err, emus) {
				...
				if (err) {
					...
					require(path.join(__dirname, 'emulators', emu.type + '.js')).start(this.config, emu, opts, function (err, emulator) {
						if (!err) {
							// TODO: emit when emulator is booted
							// adb shell getprop init.svc.bootanim

							// TODO: emit when the sdcard is ready

							// if a timeout is set and the emulator doesn't boot
							// quick enough, fire the timeout event
							if (opts.timeout) {
								var timer = setTimeout(function () {
									emulator.emit('timeout');
								}, opts.timeout);
