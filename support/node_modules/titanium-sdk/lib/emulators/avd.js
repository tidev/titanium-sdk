...
exports.isRunning = function isRunning(emu, devices, callback) {
	...
	devices = devices.filter(function (d) { return emuRegExp.test(d.id); });
	...
	devices.forEach(function (device) {
		queue.push(function (cb) {
			...
			if (m) {
				var socket = net.connect({ port: m[1] }),
					WAITING_FOR_WELCOME = 1,
					WAITING_FOR_NAME = 2,
					state = WAITING_FOR_WELCOME;

				function end(err, result) {
					if (err && err.errno) {
						console.log('avd name error');
						console.log(err);
		...
		async.whilst(
			...
			function (err) {
				...
				var args = [
					...
					'-logcat', opts.logcatFilter || '*:d,*,TiAPI:V', // enable logcat output with given tags
