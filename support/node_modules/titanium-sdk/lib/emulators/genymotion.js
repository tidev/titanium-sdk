...
exports.detect = function detect(config, opts, finished) {
	...
	async.parallel({
		...
		virtualbox: function (next) {
			appc.subprocess.findExecutable([config.get('genymotion.executables.vboxmanage'), 'VBoxManage'], function (err, result) {
				...
				if (err || !result) {
					if (process.platform == 'win32') {
						...
						appc.subprocess.run('reg', ['query', 'HKLM\\Software\\Genymobile\\VMTools-Abstract', '/v', 'VBoxPath'], function (code, out, err) {
							if (!code) {
								var m = out.match(/VBoxPath\s+REG_SZ\s+(.+)/m);
								if (m) return getVersion(m[1]);
							}
							// couldn't find it
							next(null, null);
						});
...
exports.stop = function stop(config, name, device, opts, callback) {
	...
	exports.detect(config, { bypassCache: opts.bypassCache }, function (err, results) {
		...
		exports.isRunning(emu, [ device ], function (err, running) {
			...
			// TODO: figure out a proper way to kill a Genymotion emulator (hint killing the process won't work)
