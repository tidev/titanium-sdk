...
exports.validatePlatformOptions = function (logger, config, cli, commandName) {
	var i = 0,
		len = manifest.platforms.length,
		platform,
		platformCommand;
	
	if (commandName) {
		for (; i < len; i++) {
			platform = manifest.platforms[i];
			platformCommand = path.join(path.dirname(module.filename), '..', '..', '..', platform, 'cli', 'commands', '_' + commandName + '.js');
