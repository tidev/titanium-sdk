...
exports.filterPlatforms = function (platform) {
	...
	return exports.availablePlatforms.filter(function (name) {
...
exports.validateProjectDir = function(logger, argv, name) {
	var dir = argv[name] || '.',
		rdir = argv[name] = appc.fs.resolvePath(dir);
	
	if (!appc.fs.exists(rdir)) {
		logger.banner();
		logger.error(__('Project directory does not exist') + '\n');
		process.exit(1);
	}
	
	var tiapp = path.join(rdir, 'tiapp.xml');
	while (!appc.fs.exists(tiapp) && tiapp.split(path.sep).length > 2) {
		rdir = path.dirname(rdir);
		tiapp = path.join(rdir, 'tiapp.xml');
	}
...
exports.validateCorrectSDK = function (logger, config, cli, projectDir) {
	...
	if (version.lt(sdk, '2.2.0')) { // technically, there is no 2.2, it was released as 3.0
		...
		if (platform == 'iphone') {
			...
			cmd += ' "' + projectDir + '" "' + cli.argv['deploy-type'] + '"';
		} else if (platform == 'android') {
			// TODO: what is the old python format for running an android app?
		}
	}
...
exports.validatePlatforms = function (logger, platforms) {
	var i = 0,
		...
		hasiphone = !!~platforms.indexOf('iphone'),
		hasios = !!~platforms.indexOf('ios'),
		hasipad = !!~platforms.indexOf('ipad');

	// TODO: Remove once ipad and iphone are no longer valid platforms
	if (hasios && !hasiphone && !hasipad) {
		platforms.push('iphone', 'ipad');
	}
	if ((hasiphone || hasipad) && !hasios) {
		platforms.push('ios');
	}

	for(; i < len; i++) {
		exports.validatePlatform(logger, platforms[i]);
		// TODO: remove once iphone and ipad are no longer valid platforms
	}
	return platforms;
};