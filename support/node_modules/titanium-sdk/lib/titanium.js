...
exports.availablePlatformsNames = (manifest.platforms || []).concat(Object.keys(platformAliases)).sort();
...
exports.loadPlugins = function (logger, cli, projectDir) {
	var tiapp = new exports.tiappxml(path.join(projectDir, 'tiapp.xml'));
	
	// find all hooks for active plugins
	appc.tiplugin.find(tiapp.plugins, projectDir, logger, function (plugins) {
...
exports.validateCorrectSDK = function (logger, config, cli, projectDir) {
	...
	var args = cli.argv.$_,
		p = args.indexOf('--sdk'),
		platform = platformAliases[cli.argv.platform] || cli.argv.platform,
		cmd = [],
		emulatorCmd = [],
		cmdRoot;
	...
	if (version.lt(sdk, '2.2.0')) { // technically, there is no 2.2, it was released as 3.0
		cmdRoot = 'python';
		exports.legacy.constructLegacyCommand(logger, cli, tiappxml, platform, cmd, emulatorCmd);
	} else {
		cmdRoot = 'node';
		cmd = cmd.concat(cli.argv.$0.split(' ')[1], cli.argv.$_, '--sdk', sdk);
	}
	...
	// console.log('Forking correct SDK command: ' + ( cmdRoot + ' ' +  cmd.join(' ') ).cyan + '\n');
	spawn(cmdRoot, cmd, {
		stdio: 'inherit'
	});
};
