...
function toXml(dom, parent, name, value) {
	...
	switch (name) {
		...
		case 'tizen' :
			...
			value.tizen.appid || (value.tizen.appid = randomString(10));
			node.setAttribute('appid', value.tizen.appid);
			//creating nodes from tizen specific entries
			var tizenSection = new DOMParser().parseFromString('<tizen xmlns:tizen="http://ti.appcelerator.org" appid="' + value.tizen.appid+'">\n' + value.tizen.configXml + '\n</tizen>', 'text/xml');
		...
		if (node.nodeType == xml.ELEMENT_NODE) {
			switch (node.tagName) {
				...
				case 'tizen':
					...
					//Generate application id and use default Tizen application privileges if tiapp.xml do not contains these values
					tizen.appid || (tizen.appid = randomString(10));					
					tizen.configXml || (tizen.configXml = defaultTizenConfigXml());
					break
				...
				case 'version':
					obj[node.tagName] = node.firstChild.data.replace(/\n/g, '').trim();
					break;
