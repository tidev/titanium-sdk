		...
		case 'android':
			...
			if (value.manifest) {
				dom.create('manifest', null, node, function (manifest) {
					if (value.manifest['uses-sdk']) {
						dom.create('uses-sdk', value.manifest['uses-sdk'], manifest);
					}
				...
				case 'android':
					...
					xml.forEachElement(node, function (elem) {
						switch (elem.tagName) {
							case 'manifest':
								...
								xml.forEachElement(elem, function (elem) {
									switch (elem.tagName) {
										case 'uses-sdk':
											manifest['uses-sdk'] = {};
											var tmp = xml.getAttr(elem, 'android:minSdkVersion');
											tmp && (manifest['uses-sdk']['android:minSdkVersion'] = tmp);
											tmp = xml.getAttr(elem, 'android:targetSdkVersion');
											tmp && (manifest['uses-sdk']['android:targetSdkVersion'] = tmp);
											break;
							...
							case 'proguard':
								android.proguard = !!xml.getValue(elem);
								break;

							case 'abi':
								android.abi = xml.getValue(elem);
								break;
