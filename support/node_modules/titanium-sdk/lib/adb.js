 ...
 * @param {Boolean} [dontWaitForData]
 */
Connection.prototype.exec = function exec(cmd, callback, dontWaitForData) {
	var conn = this,
		socket = this.socket,
		doSend = !!socket,
		buffer,
	...
	if (!socket) {
		...
		//console.log('SOCKET ALREADY OPEN, RE-LISTENING AND SENDING NEW COMMAND "' + cmd + '"');
	...
	socket.on('data', function (data) {
		//console.log('RECEIVED ' + data.length + ' BYTES (state=' + this.state + ') (cmd=' + cmd + ')');
		...
		switch (this.state) {
			...
			case WAIT_FOR_NEW_DATA:
				...
				if (data.length == 0) {
					//console.log('DONE, SETTING STATE TO WAIT_FOR_MORE_DATA');
					this.state = WAIT_FOR_MORE_DATA;
					dontWaitForData && callback();
					return;
				}
				...
				if (data.length >= len) {
					// yup
					if (result == 'FAIL') {
						//console.log('ERROR! ' + data.slice(0, len).toString());
						this.state = DO_NOTHING;
						callback(new Error(data.slice(0, len).toString()));
						conn.end();
					} else {
						//console.log('SUCCESS AND JUST THE RIGHT AMOUNT OF BYTES', len, data.length);
						if (dontWaitForData) {
	...
	socket.on('end', function () {
		//console.log('SOCKET CLOSED BY SERVER', (buffer && buffer.length));
		...
		return function (done) {
			...
			if (p.length > 1) {
				...
				async.whilst(
					...
					function (callback) {
						adb.shell(results.id, 'cat /system/build.prop', function (err, data) {
							if (!err && data) {
								counter = 0;
									...
									if (p != -1) {
										...
										switch (key = line.substring(0, p)) {
											...
											case 'ro.product.name':
												results[key.split('.').pop()] = value;
												break;
											case 'ro.genymotion.version':
												results.genymotion = value;
												break;
											default:
												if (key.indexOf('ro.product.cpu.abi') == 0) {
													Array.isArray(results.abi) || (results.abi = []);
													results.abi.push(value);
												}
												break;
										}
									}
...
ADB.prototype.trackDevices = function trackDevices(callback) {
	new Connection(this).exec('host:track-devices', function (err, data) {
		parseDevices(this, callback, err, data);
	}.bind(this), true);
};
...
ADB.prototype.shell = function shell(deviceId, cmd, callback) {
	...
	conn.exec('host:transport:' + deviceId, function (err, data) {
		if (err) {
			...
			conn.exec('shell:' + cmd.replace(/^shell\:/, ''), callback);
		}
	}.bind(this), true);
};
 ...
 */
ADB.prototype.installApp = function install(deviceId, apkFile, callback) {
	...
	if (!fs.existsSync(apkFile)) {
		...
		this.devices(function (err, devices) {
			...
			android.detect(this.config, null, function (results) {
				if (results.sdk && results.sdk.executables.adb) {
					appc.subprocess.run(results.sdk.executables.adb, ['-s', deviceId, 'install', '-f', apkFile], function (code, out, err) {
						if ((code && err.indexOf('No space left on device') != -1) || (!code && out.indexOf('INSTALL_FAILED_INSUFFICIENT_STORAGE') != -1)) {
...
ADB.prototype.stopApp = function stopApp(deviceId, appid, callback) {
	this.getPid(deviceId, appid, function (err, pid) {
		if (!err && pid) {
			this.shell(deviceId, 'am force-stop ' + appid, callback);
			return;
		}
