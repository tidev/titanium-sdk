			...
			targets.forEach(function (target) {
				target.split('\n\w').forEach(function (chunk) {
					...
					if (info.type == 'platform') {
						...
						sdkAidlLookup[info['api-level'] || info.id.replace('android-', '')] = info.aidl;
				...
				Object.keys(results.targets).forEach(function (id) {
					...
					if (results.targets[id].type == 'add-on' && basedOn && sdkAidlLookup[basedOn['api-level']]) {
						results.targets[id].aidl = sdkAidlLookup[basedOn['api-level']];
					}
				});
			});
