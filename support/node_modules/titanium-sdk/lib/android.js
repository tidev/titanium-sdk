...
var fs = require('fs'),
	...
	androidPackageJson = appc.pkginfo.package(module),
	...
	async.parallel({
		sdk: function (next) {
			...
			queue.push([
				...
				function (cb) {
					findSDK(config.get('android.sdkPath'), config, cb);
				},
				// try the environment variable
				function (cb) {
					findSDK(process.env.ANDROID_SDK, config, cb);
				},
				...
				function (cb) {
					findExecutable([config.get('android.executables.android'), 'android' + bat], function (err, result) {
						if (err) {
							...
							findSDK(path.resolve(result, '..', '..'), config, cb);
						}
					});
				},
		...
		results.minSDKToolsVersion         = androidPackageJson.minSDKToolsVersion;
		results.minSDKPlatformToolsVersion = androidPackageJson.minSDKPlatformToolsVersion;
		results.minSDKBuildToolsVersion    = androidPackageJson.minSDKBuildToolsVersion;
		results.minAPILevel                = ~~androidPackageJson.minAPILevel;
		results.maxAPILevel                = ~~androidPackageJson.maxAPILevel;
		results.targets                    = {};
		results.avds                       = [];
		results.issues                     = [];
		...
		if (results.linux64bit !== null) {
			...
			if (results.linux64bit.ia32libs === false) {
				results.issues.push({
					...
					type: 'warning',
					message: __('ia32 libraries is not installed.') + '\n'
						+ __('Without the ia32 libraries, the Android Emulator will not work properly.') + '\n'
						+ __('To install the required ia32 libraries, run "%s".', '__sudo apt-get install ia32-libs__')
				});
			}
			...
			targets.forEach(function (target) {
				target.split('\n\w').forEach(function (chunk) {
					...
					if (info.type == 'platform') {
						...
						info.supported = !~~info['api-level'] || info['api-level'] < results.minAPILevel ? false : info['api-level'] - 0 <= results.maxAPILevel ? true : 'maybe';
					} else if (info.type == 'add-on' && info['based-on']) {
						info.path = addons[info.name + '|' + info.vendor + '|' + info['based-on']['api-level'] + '|' + info.revision] || null;
						info.supported = !~~info['based-on']['api-level'] || info['based-on']['api-level'] < results.minAPILevel ? false : info['based-on']['api-level'] - 0 <= results.maxAPILevel ? true : 'maybe';
					}
					...
					if (!info.supported) {
						results.issues.push({
							...
							message: __('Android API %s is too old and is no longer supported by Titanium SDK %s.', '__' + info.name + ' (' + info.id + ')__', manifestJson.version) + '\n' +
								__('The minimum supported Android API level by Titanium SDK %s is API level %s.', manifestJson.version, results.minAPILevel)
						});
						...
						results.issues.push({
							...
							message: __('Android API %s is too new and may or may not work with Titanium SDK %s.', '__' + info.name + ' (' + info.id + ')__', manifestJson.version) + '\n' +
								__('The maximum supported Android API level by Titanium SDK %s is API level %s.', manifestJson.version, results.maxAPILevel)
						});
					}
				});
			});
...
function findSDK(dir, config, callback) {
			...
			if (fs.statSync(path.join(buildToolsDir, ver)).isDirectory() && fs.existsSync(file)) {
				...
				if (m) {
					result.buildTools = {
						...
						supported: appc.version.gte(m[1], androidPackageJson.minSDKBuildToolsVersion),
						version: m[1]
					};
					var file;
		...
		if (fs.existsSync(file)) {
			...
			if (m) {
				result.tools = {
					...
					supported: appc.version.gte(m[1], androidPackageJson.minSDKToolsVersion),
					version: m[1]
				};
			}
		}
		...
		if (fs.existsSync(file)) {
			...
			if (m) {
				result.platformTools = {
					...
					supported: appc.version.gte(m[1], androidPackageJson.minSDKPlatformToolsVersion),
					version: m[1]
				};
			}
		}
