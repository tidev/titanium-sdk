...
import string, subprocess, re, zipfile
...
class Android(object):
	...
	def build_modules_info(self, resources_dir, app_bin_dir):
		...
		for jar in os.listdir(android_modules_dir):
			...
			module_jar = zipfile.ZipFile(os.path.join(android_modules_dir, jar))
			bindings_json = module_jar.read('org/appcelerator/titanium/gen/bindings.json')
			module_bindings = simplejson.loads(bindings_json)
			for module_name in bindings['modules'].keys():
				modules[module_name] = module_bindings['modules'][module_name]
		...
		for module in compiler.modules:
			...
			self.app_modules.append({
				'api_name': module,
				# TODO: this should come from a lookup table generated by the annotation processor
				'class_name': modules[module]['className'],
				'bindings': bindings
			})
