...
var result = fooBarList!=null && fooBarList.length == 1 && fooBarList.item(0).firstChild.nodeValue=="true";
result = result && fooBarList.item(0).nodeName.toLowerCase() == "foobar";
...
testResult = fooBarList!=null && fooBarList.length == 1 && fooBarList.item(0).firstChild.nodeValue == "true";
testResult = testResult && fooBarList.item(0).nodeName.toLowerCase() == "foobar";

//TODO: remove when XPath is supported in android
if (xml2.evaluate) {
	// test XPath against xml
...
function nodewalker(node) 
{
	...
	if (1 == node.nodeType || 2 == node.nodeType || 9 == node.nodeType || 11 == node.nodeType) { 
		nodeCount++;
	}
...
testResult = testResult && nodeCount==9;
...
Ti.API.info("Script is: "+scriptList);

testResult = testResult && scriptList.length==1;
testResult = testResult && xml.documentElement.nodeName == "root";

Ti.API.info('>>>> Test 5 Passed before NodeCount: '+testResult);

nodeCount = 0;
nodewalker(scriptList.item(0));
...
// Fix some trobles in webkit with JSON.stringify
if ('Chrome' == Ti.Platform.name || 'Safari' == Ti.Platform.name) {
	Ti.API.info("Script is: "+Ti.XML.serializeToString(subdataList));
} else {
	Ti.API.info("Script is: "+JSON.stringify(subdataList));
}

testResult = testResult && dataList.length==1;
testResult = testResult && xml.documentElement.nodeName == "data";
Ti.API.info('>>>> Test 6 Passed before NodeCount: '+testResult);

nodeCount = 0;
nodewalker(subdataList.item(0));
testResult = testResult && nodeCount==3;
