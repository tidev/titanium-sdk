	...
	async.parallel({
		...
		if (results.linux64bit !== null) {
			...
			if (missing32bitLibs.length) {
				results.issues.push({
					...
					type: 'error',
					message: __('Missing 32-bit libraries is not installed.') + '\n'
		...
		if (process.platform == 'win32' && results.sdk.path.indexOf('&') != -1) {
			results.issues.push({
				...
				type: 'error',
				message: __('The Android SDK path must not contain ampersands (&).')
			});
			results.sdk = null;
			return finalize();
		}
		...
		run(results.sdk.executables.android, 'list', { cwd: afs.resolvePath('~') }, function (err, stdout, stderr) {
			...
			var sections = {};
			stdout.split(/\n\n|Available Android Virtual Devices\:/).forEach(function (chunk) {
				var lines = chunk.split('\n'),
					title = lines.shift();
				sections[title || 'Available Android Virtual Devices:'] = lines.join('\n').trim().split(/\-\-\-\-\-\-+\n/);
			});
