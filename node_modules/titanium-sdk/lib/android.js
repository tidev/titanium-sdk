...
function findNDK(dir, config, callback) {
	...
	var things = ['ndk-build' + cmd, 'build', 'prebuilt', 'platforms'];
	if (!things.every(function (thing) { return fs.existsSync(path.join(dir, thing)); })) {
		return callback(true);
	}

	// try to determine the version
	var version;
	var sourceProps = path.join(dir, 'source.properties');
	if (fs.existsSync(sourceProps)) {
		var m = fs.readFileSync(sourceProps).toString().match(/Pkg\.Revision\s*=\s*(.+)/m);
		if (m && m[1]) {
			version = m[1].trim();
		}
	}
...
function loadPlatform(dir, systemImages) {
	...
	var sourceProps = readProps(path.join(dir, 'source.properties'));
	var apiLevel = sourceProps ? ~~sourceProps['AndroidVersion.ApiLevel'] : null;
	if (!sourceProps || !apiLevel || !isFile(path.join(dir, 'build.prop'))) {
		return null;
	}

	// read in the sdk properties, if exists
	var sdkProps = readProps(path.join(dir, 'sdk.properties'));

	// detect the available skins
	var skinsDir = path.join(dir, 'skins');
	var skins = isDir(skinsDir) ? fs.readdirSync(skinsDir).map(name => {
		return isFile(path.join(skinsDir, name, 'hardware.ini')) ? name : null;
	}).filter(x => x) : [];
	var defaultSkin = sdkProps && sdkProps['sdk.skin.default'];
	if (skins.indexOf(defaultSkin) === -1 && skins.indexOf(defaultSkin = 'WVGA800') === -1) {
		defaultSkin = skins[skins.length - 1] || null;
	}

	var apiName = sourceProps['AndroidVersion.CodeName'] || apiLevel;
	var id = `android-${apiName}`;
	var tmp;
