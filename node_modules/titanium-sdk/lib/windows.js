...
exports.detect = function detect(config, opts, finished) {
	...
	async.parallel({
		'visualstudio': function (next) {
			...
			async.parallel([
				...
				if (!Object.keys(possibleVersions).length) {
					results.issues.push({
						id: 'WINDOWS_VISUAL_STUDIO_NOT INSTALLED',
						type: 'error',
		...
		'windowsphone': function (next) {
			...
			async.parallel([
				...
				if (!Object.keys(wpInfo).length) {
					results.issues.push({
						id: 'WINDOWS_PHONE_SDK_NOT INSTALLED',
						type: 'error',
						message: __('Microsoft Windows Phone SDK not installed.') + '\n' +
							__('You will be unable to build Windows Phone apps.') + '\n' +
							__('You can install it from %s.', '__http://appcelerator.com/windowsphone__')
		...
		var selectedVisualStudio = results.visualstudio[Object.keys(results.visualstudio).filter(function (v) {
				return results.visualstudio[v].selected;
			}).pop()],
			selectedWindowsPhoneSDK = results.windowsphone[Object.keys(results.windowsphone).filter(function (v) {
				return results.windowsphone[v].selected;
			}).pop()];
		...
		async.parallel({
			'msbuild': function (next) {
				...
				appc.subprocess.getRealName(selectedVisualStudio.vcvarsall, function (err, vcvarsall) {
					if (err) return next();
