...
exports.isRunning = function isRunning(config, emu, devices, callback) {
	...
	var queue = async.queue(function (task, next) {
		task(function (err, result) {
			if (err) {
				next(); // go to next item in the queue
			} else {
...
exports.isEmulator = function isEmulator(config, device, callback) {
	var port = device.match(/^emulator\-(\d+)$/);
...
exports.start = function start(config, emu, opts, callback) {
	...
	android.detect(config, { bypassCache: opts.bypassCache }, function (results) {
		if (!results.sdk) return callback(new Error(__('No Android SDK found')));
		...
		async.whilst(
			...
			function (err) {
				...
				var child = this.child = spawn(results.sdk.executables.emulator, args, emuopts),
