...
Builder.prototype.run = function run(logger, config, cli, finished) {
	...
	var patterns = cli.tiapp['ignore-files'];
	if (patterns && patterns.length > 0) {
		var tiappIgnoreFiles = '^(' + patterns.join('|').replace(/\./g, '\\.').replace(/\*/g, '.*') + ')$';
		this.ignoreFiles = new RegExp(this.ignoreFiles.source + '|' + tiappIgnoreFiles);
	}
