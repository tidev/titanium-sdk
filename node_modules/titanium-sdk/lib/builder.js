...
Builder.prototype.generateAppIcons = function generateAppIcons(icons, callback) {
	var defaultIcon = path.join(this.projectDir, 'DefaultIcon.png'),
		...
		fail = function () {
			...
			icons.forEach(function (icon) {
				if (size === null || icon.width > size.width) {
					this.logger.error('  ' +
						__('%s - required size: %sx%s',
							icon.description,
							icon.width,
							icon.height
						)
	...
	if (!fs.existsSync(defaultIcon)) {
		this.logger.error(__('There are missing app icons'));
	...
	if (size.width !== size.height) {
		this.logger.error(__('The DefaultIcon.png is %sx%s, however the width and height must be equal', size.width, size.height));
		this.logger.error(__('It is highly recommended that the DefaultIcon.png be 1024x1024'));
		return fail();
	}

	this.logger.debug(__('Found %s (%sx%s)', 'DefaultIcon.png'.cyan, size.width, size.height));
	...
	for (i = 0; i < icons.length; i++) {
		...
		if (icon.required) {
			...
			this.logger.warn(__('DefaultIcon.png (%sx%s) is not large enough to generate missing icon "%s" (%sx%s), skipping', size.width, size.height, path.basename(icon.file), icon.width, icon.height));
			icons.splice(i--, 1);
			continue;
		}
	...
	if (minRequiredSize !== null && size.width < minRequiredSize) {
		this.logger.error(__('The DefaultIcon.png must be at least %sx%s', minRequiredSize, minRequiredSize));
		this.logger.error(__('It is highly recommended that the DefaultIcon.png be 1024x1024'));
		return fail();
	}
