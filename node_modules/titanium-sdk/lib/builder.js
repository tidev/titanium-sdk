 ...
 * @param {Boolean} [forceSymlink] - When true, forces the file to be symlinked.
 */
Builder.prototype.copyFileSync = function copyFileSync(src, dest, forceCopy) {
	...
	if (exists) {
		this.logger.warn(__('Overwriting file %s', dest.cyan));
		fs.unlinkSync(dest);
	}

	if (!forceCopy && this.symlinkFilesOnCopy) {
	...
	(function copy(src, dest, isRootDir) {
		...
		fs.readdirSync(src).forEach(function (name) {
			...
			if (typeof opts.callback === 'function') {
				var result = opts.callback(srcFile, destFile, srcStat);
				if (result === null) {
					return; // skip
			...
			if (fs.existsSync(destFile)) {
				this.logger.warn(__('Overwriting file %s', destFile.cyan));
				fs.unlinkSync(destFile);
			}

			if (opts.forceCopy !== true && (this.symlinkFilesOnCopy || opts.forceSymlink)) {
				this.logger.debug(__('Symlinking %s => %s', srcFile.cyan, destFile.cyan));
				fs.symlinkSync(srcFile, destFile);
			} else {
				this.logger.debug(__('Copying %s => %s', srcFile.cyan, destFile.cyan));
