	...
	return magik(options, callback, function (emitter, options, callback) {
		...
		xcode.detect(options, function (err, xcodeInfo) {
			...
			detect(options, function (err, simInfo) {
				...
				// helper class for buffering and handling output from ios-sim.
				// we split the data by line and print everything except the most recent line.
				function OutputBuffer(stream) {
					var _t = this;
					this.buffer = '';

					stream.on('data', function (data) {
						var buffer = (_t.buffer + data.toString()).split('\n');
						_t.buffer = buffer.pop();
						for (var i = 0, len = buffer.length; i < len; i++) {
							_t.emit('line', buffer[i]);
						}
					});
				}

				util.inherits(OutputBuffer, events.EventEmitter);

				OutputBuffer.prototype.flush = function flush() {
					var buffer = this.buffer.split('\n');
					this.buffer = '';
					for (var i = 0, len = buffer.length; i < len; i++) {
						var s = buffer[i].trim();
						s && this.emit('line', s);
				...
				function launchSim() {
					...
					if (options.launchWatchApp || options.launchWatchAppOnly) {
						if (options.launchWatchApp) {
							...
							args.push('--launch-watch-app-only');
						}
					...
					var stdoutBuffer,
						stderrBuffer = new OutputBuffer(simProcess.stderr);
					stderrBuffer.on('line', function (line) {
						emitter.emit('log-debug', '[ios-sim] ' + line);
						if (line.indexOf('Simulator started successfully') !== -1) {
							emitter.emit('launched', simHandle);
						} else if (line.indexOf('App launched successfully') !== -1) {
					...
					if (appName) {
						...
						stdoutBuffer = new OutputBuffer(simProcess.stdout);
						stdoutBuffer.on('line', function (line) {
							...
							options.autoExit && line.indexOf(autoExitToken) !== -1 && stop(simHandle);
						});
					} else {
					...
					simProcess.on('close', function (code, signal) {
						...
						// flush buffers
						stdoutBuffer && stdoutBuffer.flush();
						stderrBuffer.flush();
