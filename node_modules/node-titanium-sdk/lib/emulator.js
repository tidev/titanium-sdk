 ...
 * @param {Function} callback - A function to call when the detection has completed
 */
EmulatorManager.prototype.isRunning = function isRunning(id, opts, callback) {
	if (opts && typeof opts == 'function') {
		callback = opts;
		opts = {};
	}

	opts.logger && opts.logger.trace(__('Detecting if %s exists...', id.cyan));
...
function checkedBooted(config, opts, emulator) {
	...
	conn = adb.trackDevices(function (err, devices) {
		if (err) {
			opts.logger && opts.logger.trace(__('Error tracking devices: %s', err.message));
			return;
 ...
 */
EmulatorManager.prototype.start = function start(id, opts, callback) {
	if (opts && typeof opts == 'function') {
		callback = opts;
		opts = {};
	}

	opts.logger && opts.logger.trace(__('Checking if emulator %s is running...', id.cyan));

	this.isRunning(id, opts, function (err, running) {
		if (err) {
			// something went boom
			return callback(err);
		}
 ...
 */
EmulatorManager.prototype.stop = function stop(id, opts, callback) {
	if (opts && typeof opts == 'function') {
		callback = opts;
		opts = {};
	}
