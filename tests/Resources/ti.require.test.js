...
describe.only('requireJS', function () {
	...
	it('require should not fail when exports is falsey', function () {
		var files = [
			{ filename: 'empty-double', expected: 0 },
			{ filename: 'empty-single', expected: 0 },
			{ filename: 'false', expected: 0 },
			{ filename: 'nan', expected: 0 },
			{ filename: 'null', expected: 0 },
		...
		for (var obj in files ) {
			...
			should(result).be.eql(obj.expected);
		}
	});
});
