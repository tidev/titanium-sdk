...
public abstract class TiApplication extends Application implements KrollApplication
{
	...
	private String tiBuildTimestamp;
	...
	{
		if (this.tiBuildTimestamp == null) {
			// The timestamp is provided by an asset file generated by the SDK build. (See "generateTiBuildInfo"
			// task in "build.gradle".) It's not a "BuildConfig" field so Java builds can stay incremental.
			String value = "";
			try (InputStream stream = getAssets().open("ti-build-info.properties")) {
				Properties properties = new Properties();
				properties.load(stream);
				value = properties.getProperty("buildTime", "").trim();
			} catch (Exception ex) {
				Log.w(TAG, "Failed to read Titanium build timestamp. Reason: " + ex.getMessage());
			}
			this.tiBuildTimestamp = value;
		}
		return this.tiBuildTimestamp;
	}
