...
@Kroll.proxy(propertyAccessors = { TiC.PROPERTY_EXIT_ON_CLOSE, TiC.PROPERTY_FULLSCREEN, TiC.PROPERTY_TITLE,
		TiC.PROPERTY_TITLEID, TiC.PROPERTY_WINDOW_SOFT_INPUT_MODE })
public abstract class TiWindowProxy extends TiViewProxy
{
	...
	/**
	 * Called when the window gained or lost focus. Default implementation will fire "focus" and "blur" events when the
	...
	/**
	 * Sub-classes will need to call handlePostOpen after their window is visible so any pending dialogs can
	 * successfully show after the window is opened
	 */
	...
	public int getOrientation()
	{
		...
		if (activity != null) {
			return TiOrientationHelper
					.convertConfigToTiOrientationMode(activity.getResources().getConfiguration().orientation);
		}
