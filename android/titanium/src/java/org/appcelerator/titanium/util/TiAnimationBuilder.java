...
/**
 * Builds and starts animations. When possible, Honeycomb+ animations (i.e., Property Animators) are used, even on
 * pre-Honeycomb, which is possible because we use the NineOldAndroids compatibility library. The only time Property
 * Animators are not used -- i.e., the only time we revert to using old-style view animations -- is when the animation's
 * transform property is set to a Ti2DMatrix that is "complicated", where complicated means one of the following is
 * true:
 * <ol>
 ...
 * 
 * </li>
 * <li>The Ti2DMatrix is the result of concatenating an operation more than once, e.g.,
 * 
 * <pre>
 * {
 * 	&#064;code
 ...
 * </pre>
 * 
 * which contains two rotates.</li> </ul> Concatenation of matrix operations does not in itself cause old style
 ...
 * </pre>
 * 
 * ...would be fine since no operation is duplicated. In such a case, a set of Honeycomb+ property Animators can be
 * safely derived.
 */
public class TiAnimationBuilder
{
	...
	/**
	 * Builds the Animations used for pre-Honeycomb style of animations, i.e., view animations instead of property
	 * animations. We use this only when we can't use NineOldAndroids to accomplish the animation, which happens only
	 * when the animation contains a transform (Ti2DMatrix) that is too complicated. See this class documentation for
	 * more information.
	 * 
	 * @return An AnimationSet with the view Animations that will be used to accomplish the requested animation.
	 */
	 ...
	 * 
	 * @return AnimatorSet containing the Animator instances that will be used to accomplish this animation.
	 */
	...
	/**
	 * Builds the Animators used for Honeycomb+ animations (i.e., property animation instead of view animation.)
	 * 
	...
	private AnimatorSet buildPropertyAnimators(int x, int y, int w, int h, int parentWidth, int parentHeight)
	{
		...
		if (top != null || bottom != null || left != null || right != null || centerX != null || centerY != null) {
			...
			if (PRE_HONEYCOMB) {
				...
				// For Honeycomb+, animatorHelper will reset layout parameters so the layout will keep correct
				// (TIMOB-15951).
			} else {
	...
	private AnimationSet buildViewAnimations(int x, int y, int w, int h, int parentWidth, int parentHeight)
	{
		...
		if (top != null || bottom != null || left != null || right != null || centerX != null || centerY != null) {
			...
			if (Log.isDebugModeEnabled()) {
				Log.d(TAG, "animate " + viewProxy + " relative to self: " + (horizontal[0] - x) + ", "
						+ (vertical[0] - y), Log.DEBUG_MODE);
			}
	...
	public static class TiMatrixAnimation extends Animation
	{
		...
		public void invalidateWithMatrix(View view)
		{
			...
			if (view.getParent() instanceof ViewGroup) {
				...
				((ViewGroup) view.getParent()).invalidate(left + rect.left, top + rect.top, left + rect.width(), top
						+ rect.height());
			}
		}
	}
	...
	/**
	 * A helper class for Honeycomb+ Property Animators to animate width/height/top/bottom/left/right/center. Based on
	 * the Android doc http://developer.android.com/guide/topics/graphics/prop-animation.html, to have the
	 * ObjectAnimator update properties correctly, the property must have a setter function.
	 */
		...
		public void onAnimationEnd(Animator animator)
		{
			if (relayoutChild) {
				...
				for (Object key : options.keySet()) {
					if (TiC.PROPERTY_TOP.equals(key) || TiC.PROPERTY_BOTTOM.equals(key)
							|| TiC.PROPERTY_LEFT.equals(key) || TiC.PROPERTY_RIGHT.equals(key)
							|| TiC.PROPERTY_CENTER.equals(key) || TiC.PROPERTY_WIDTH.equals(key)
							|| TiC.PROPERTY_HEIGHT.equals(key) || TiC.PROPERTY_BACKGROUND_COLOR.equals(key)) {
	...
	/**
	 * Assemble and start (or schedule if delayed) the animation on the given view.
	 * 
	...
	/**
	 * Determine whether an animation is currently running on the given view by checking the static map of views with
	 * running animations.
	 * 
	 * @param v The native View.
	 * @return true if the map of running animations contains the View, false otherwise.
	 */
	...
	/**
	 * Indicate that an animation is currently running on the given view.
	 * 
	 * @param v The native View.
	 */
	...
	/**
	 * Indicate that an animation is or is not running for a given view by either adding it to or removing it from a map
	 * that tracks which views have animations currently running.
	 * 
	 * @param v The native View.
