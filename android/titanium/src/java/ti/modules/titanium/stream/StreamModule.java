...
public class StreamModule extends KrollModule
{
	...
	// public void read(TiStream sourceStream, BufferProxy buffer, int offset, int length, KrollFunction
	// resultsCallback)
		...
		if (args.length == 1) {
			...
			new Thread(new Runnable()
			{
				public void run()
				{
					...
					fResultsCallback.callAsync(getKrollObject(),
							buildRWCallbackArgs(fsourceStream, fbuffer.getLength(), errorState, errorDescription));
				}
			}).start();
	...
	// public void write(TiStream outputStream, BufferProxy buffer, int offset, int length, KrollFunction
	// resultsCallback)
	public void write(Object args[])
	{
		...
		new Thread(new Runnable()
		{
			public void run()
			{
				...
				fResultsCallback.callAsync(getKrollObject(),
						buildRWCallbackArgs(foutputStream, bytesWritten, errorState, errorDescription));
			}
		}).start();
	}
	...
	// public void writeStream(TiStream inputStream, TiStream outputStream, int maxChunkSize, KrollFunction
	// resultsCallback)
			...
			new Thread(new Runnable()
			{
				public void run()
				{
					...
					fResultsCallback.callAsync(
							getKrollObject(),
							buildWriteStreamCallbackArgs(finputStream, foutputStream, totalBytesWritten, errorState,
									errorDescription));
				}
			}).start();
	...
	private void pump(TiStream inputStream, KrollFunction handler, int maxChunkSize)
	{
		...
		try {
			while (true) {
				...
				handler.call(
						getKrollObject(),
						buildPumpCallbackArgs(inputStream, buffer, bytesRead, totalBytesRead, errorState,
								errorDescription));
				buffer = null;
			...
			handler.call(
					getKrollObject(),
					buildPumpCallbackArgs(inputStream, new BufferProxy(), 0, totalBytesRead, errorState,
							errorDescription));
		}
	}
