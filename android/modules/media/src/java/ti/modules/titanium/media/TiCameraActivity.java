...
public class TiCameraActivity extends TiBaseActivity implements
	...
	private static class PreviewLayout extends FrameLayout {
		...
		protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
			...
			optimalPreviewSize = getOptimalPreviewSize(supportedPreviewSizes,
					previewWidth, previewHeight);
			optimalVideoSize = getOptimalPreviewSize(supportedVideoSizes,
					previewWidth, previewHeight);
			if (optimalPreviewSize != null) {
				if (previewWidth > previewHeight) {
					aspectRatio = (double) optimalPreviewSize.width
							/ optimalPreviewSize.height;
				} else {
					aspectRatio = (double) optimalPreviewSize.height
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		cameraLayout.addView(previewLayout, new FrameLayout.LayoutParams(
	...
	public void surfaceCreated(SurfaceHolder previewHolder) {
		try {
			...
			onError(MediaModule.UNKNOWN_ERROR,
					"Unable to setup preview surface: " + e.getMessage());
			cancelCallback = null;
			finish();
			return;
		}
	...
	protected void onPause() {
		...
		cameraLayout.removeView(localOverlayProxy.getOrCreateView()
				.getNativeView());
	...
	private void startPreview(SurfaceHolder previewHolder) {
		...
		if (supportedFocusModes
				.contains(MediaModule.FOCUS_MODE_CONTINUOUS_PICTURE)) {
		...
		if (optimalPreviewSize != null) {
			param.setPreviewSize(optimalPreviewSize.width,
					optimalPreviewSize.height);
		}
	...
	static public void startVideoCapture() {
		...
		try {
			...
			onError(MediaModule.UNKNOWN_ERROR,
					"Unable to unlock camera: " + e.getMessage());
			return;
		}
		...
		CamcorderProfile profile = CamcorderProfile.get(whichCamera,
				videoQuality);
		...
		try {
			...
			onError(MediaModule.UNKNOWN_ERROR, "Unable to prepare recorder: "
					+ e.getMessage());
			return;
		}
		...
		try {
			...
			onError(MediaModule.UNKNOWN_ERROR, "Unable to start recording: "
					+ e.getMessage());
			return;
		}
	}
	...
	static public void stopVideoCapture() {
		...
		try {
			...
			onError(MediaModule.UNKNOWN_ERROR,
					"Unable to reconnect to camera after recording: "
							+ e.getMessage());
		}

		try {
			if (successCallback != null) {
				TiFile theFile = new TiFile(videoFile, videoFile.toURI()
						.toURL().toExternalForm(), false);
				TiBlob theBlob = TiBlob.blobFromFile(theFile);
				KrollDict response = MediaModule.createDictForImage(theBlob,
						theBlob.getMimeType());
			...
			if (errorCallback != null) {
				...
				response.putCodeAndMessage(MediaModule.UNKNOWN_ERROR,
						t.getMessage());
	...
	@Override
	public void finish() {
		// For API 10 and above, the whole activity gets destroyed during an
		// orientation change. We only want to set
		// overlayProxy to null when we call finish, i.e. when we hide the
		// camera or take a picture. By doing this, the
		// overlay proxy will be available during the recreation of the activity
	...
	/**
	 * Computes the optimal preview size given the target display size and
	 * aspect ratio.
	 * 
	...
	/**
	 * Computes the optimal picture size given the preview size. This returns
	 * the maximum resolution size.
	 * 
	 * @param sizes
				...
				if (!(focusMode.equals(Parameters.FOCUS_MODE_EDOF)
					...
					AutoFocusCallback focusCallback = new AutoFocusCallback() {
						public void onAutoFocus(boolean success, Camera camera) {
							...
							// "cancelAutoFocus" causes the camera to crash on M
							// (probably due to discontinued support of
							// android.hardware.camera)
							// We need to move to android.hardware.camera2 APIs
							// as soon as we can.
	...
	static PictureCallback jpegCallback = new PictureCallback() {
		public void onPictureTaken(byte[] data, Camera camera) {
			try {
				...
				if (successCallback != null) {
					TiFile theFile = new TiFile(imageFile, imageFile.toURI()
							.toURL().toExternalForm(), false);
					TiBlob theBlob = TiBlob.blobFromFile(theFile);
					KrollDict response = MediaModule.createDictForImage(
							theBlob, theBlob.getMimeType());

					// add previewRect to response
					KrollDict previewRect = new KrollDict();
					if (optimalPreviewSize != null) {
						previewRect.put(TiC.PROPERTY_WIDTH,
								optimalPreviewSize.width);
						previewRect.put(TiC.PROPERTY_HEIGHT,
								optimalPreviewSize.height);
					} else {
						previewRect.put(TiC.PROPERTY_WIDTH, 0);
						previewRect.put(TiC.PROPERTY_HEIGHT, 0);
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_MENU) {
			// Workaround for
			// http://code.google.com/p/android/issues/detail?id=61394
