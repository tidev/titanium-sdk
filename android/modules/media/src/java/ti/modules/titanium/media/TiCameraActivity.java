...
public class TiCameraActivity extends TiBaseActivity implements SurfaceHolder.Callback, MediaRecorder.OnInfoListener
{
	...
	// before the built in camera will open
	...
	public void finish()
	{
		...
		// orientation change. We only want to set overlayProxy to null when we call
		// finish, i.e. when we hide the camera or take a picture. By doing this,
		// the overlay proxy will be available during the recreation of the activity
	...
	/**
   * Computes the optimal preview size given the target display size and aspect
   * ratio.
   *
	...
	private static File writeToFile(byte[] data, boolean saveToGallery) throws Throwable
	{
		try {
			...
			if (saveToGallery) {
				...
				// Save the picture in the internal data directory so it is private to
				// this application.
				...
				if (!(focusMode.equals(Parameters.FOCUS_MODE_EDOF) || focusMode.equals(Parameters.FOCUS_MODE_FIXED)
					...
					AutoFocusCallback focusCallback = new AutoFocusCallback() {
						public void onAutoFocus(boolean success, Camera camera)
						{
							...
							// "cancelAutoFocus" causes the camera to crash on M (probably due
							// to discontinued support of android.hardware.camera) We need to
							// move to android.hardware.camera2 APIs as soon as we can.
