...
import ti.modules.titanium.facebook.FBActivity;
import ti.modules.titanium.facebook.FBActivityDelegate;
import ti.modules.titanium.facebook.FBDialog;
import ti.modules.titanium.facebook.FBFeedDialog;
import ti.modules.titanium.facebook.FBLoginDialog;
import ti.modules.titanium.facebook.FBPermissionDialog;
import ti.modules.titanium.facebook.FBRequest;
import ti.modules.titanium.facebook.FBUtil;
import ti.modules.titanium.facebook.FBSession;
import ti.modules.titanium.facebook.FBStreamDialog;
...
public class FacebookModule extends TiModule implements FBActivityDelegate, TiActivityResultHandler
{
	...
	private Map<Integer,KrollCallback> callbacks = new HashMap<Integer,KrollCallback>(1);
	...
	FBSession getOrCreateSession(String key, String secret, String sessionProxy)
	{
		...
		{
			this.session = FBSession.getSessionForApplication_getSessionProxy(key, sessionProxy, new FBSessionDelegateImpl(), this);
		}
		else
		{
			this.session = FBSession.getSessionForApplication_secret(key, secret, new FBSessionDelegateImpl(), this);
		}
	...
	public boolean setup(String key, String secret, String sessionProxy, KrollCallback callback)
	{
		Log.d(LCAT,"setup called with key: "+key+", secret: "+secret+", sessionProxy: "+sessionProxy+", callback: "+callback);
		...
		if (loggedIn)
		{
			this.session.setContext(w.getContext());
			triggerLoginChange();
		}
	...
	public void execute(String method, Map<String,String> params, KrollCallback callback, Object dataObj)
	{
	...
	public void logout(KrollCallback callback)
	{
		...
		if (isLoggedIn())
		{
			...
			getTiContext().getActivity().runOnUiThread(new Runnable() {
				public void run() {
	...
	public void publishStream(String title, TiDict data, String target, KrollCallback callback)
	{
	...
	public void publishFeed(long templateBundleId, String data, String body, KrollCallback callback)
	{
		Activity activity = getTiContext().getActivity();
		TiActivitySupport activitySupport = (TiActivitySupport) activity;
		final int resultCode = activitySupport.getUniqueResultCode();
...
//		intent.putExtra("userMessagePrompt",userMessagePrompt);
	...
	private void triggerLogIn(TiDict event)
	{
		...
		if (setupCallback!=null) setupCallback.callWithProperties(event);
	}
   ...
   public FBDialog onCreate(String action, Activity activity, Bundle state)
   {
	...
	public void triggerLoginChange(boolean showDialog)	 
	{
		...
		String fql2 = "select status_update,photo_upload,sms,email,create_event,rsvp_event,publish_stream,read_stream,share_item,create_note from permissions where uid == " + session.getUid();
   ...
   private final class FBQueryRequestDelegateImpl extends FBRequestDelegate
   {
	    ...
	    {
			Log.d(LCAT,"received request result = "+result);
			Log.d(LCAT,"result type: "+result.getClass().getName());
			TiDict event = new TiDict();
			event.put("success", true);
