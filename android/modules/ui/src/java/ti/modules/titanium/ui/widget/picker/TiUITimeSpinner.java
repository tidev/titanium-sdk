...
public class TiUITimeSpinner extends TiUIView implements WheelView.OnItemSelectedListener
{
	...
	public void setValue(long value, boolean suppressEvent)
	{
		...
		ignoreItemSelection = false;
		minutesWheel.setCurrentItem(
			((FormatNumericWheelAdapter) minutesWheel.getAdapter()).getIndex(calendar.get(Calendar.MINUTE)));
		suppressChangeEvent = false;
	}
	...
	public void onItemSelected(WheelView view, int index)
	{
		...
		calendar.set(Calendar.MINUTE,
					 ((FormatNumericWheelAdapter) minutesWheel.getAdapter()).getValue(minutesWheel.getCurrentItem()));
		if (!format24) {
			int hourOfDay = 0;
