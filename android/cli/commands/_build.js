...
import android from 'node-titanium-sdk/lib/android.js';
import { detect as androidDetect } from '../lib/detect.js';
...
import i18n from 'node-titanium-sdk/lib/i18n.js';
...
class AndroidBuilder extends Builder {
					...
					appc.jdk.detect(config, null, function (jdkInfo) {
						...
						if (!version.satisfies(jdkInfo.version, _t.packageJson.vendorDependencies.java)) {
							logger.error(`JDK version ${
								...
								_t.packageJson.vendorDependencies.java
							} is supported\n`);
							process.exit(1);
						}
						...
						'device-id': {
							...
							validate: function (device, callback) {
								...
								findTargetDevices(cli.argv.target, function (err, devices) {
									...
									callback(new Error(`Invalid Android ${cli.argv.target ? 'device' : 'emulator'} "${device}"`));
								});
							},
										...
										if (avds.length) {
											...
											if (avds.length === 1) {
												...
												if (!cli.argv['avd-skin']) {
													...
													if (avds.length === 0) {
														logger.error(`No emulators found with id "${
															cli.argv['avd-id']
														}" and skin "${
															cli.argv['avd-skin']
														}"\n`);
										...
										appc.subprocess.run(_t.jdkInfo.executables.keytool, [
											...
											if (code) {
												if (out.indexOf('java.security.UnrecoverableKeyException') !== -1) {
													return callback(new Error('Bad key password'));
												}
									...
									if (keystoreFile && _t.jdkInfo && _t.jdkInfo.executables.keytool) {
										appc.subprocess.run(_t.jdkInfo.executables.keytool, [
											...
											if (_t.keystoreAliases.length === 0) {
												cli.argv.keystore = undefined;
												return callback(new Error('Keystore does not contain any certificates'));
	...
	async validate(logger, config, cli) {
		...
		if (cli.tiapp.properties['ti.deploytype']) {
			logger.warn(`The ${
				'ti.deploytype'.cyan
			} tiapp.xml property has been deprecated, please use the ${
				'--deploy-type'.cyan
			} option`);
		}
		...
		if (cli.tiapp.properties['ti.android.compilejs']) {
			logger.warn(`The ${
				...
				'--skip-js-minify'.cyan
			} option to bypass JavaScript minification`);
		}
		...
		if (cli.tiapp.name.indexOf('&') !== -1) {
			if (config.get('android.allowAppNameAmpersands', false)) {
				logger.warn(`The app name "${cli.tiapp.name}" contains an ampersand (&) which will most likely cause problems.`);
				logger.warn('It is recommended that you define the app name using i18n strings.');
				logger.warn(`Refer to ${
					'https://titaniumsdk.com/guide/Titanium_SDK/Titanium_SDK_How-tos/Cross-Platform_Mobile_Development_In_Titanium/Internationalization.html'.cyan
				} for more information.`);
			} else {
				logger.error(`The app name "${
					cli.tiapp.name
				}" contains an ampersand (&) which will most likely cause problems.`);
				logger.error('It is recommended that you define the app name using i18n strings.');
				logger.error(`Refer to ${
		...
		if (!config.get('app.skipAppIdValidation') && !cli.tiapp.properties['ti.skipAppIdValidation']) {
			if (!/^([a-zA-Z_]{1}[a-zA-Z0-9_-]*(\.[a-zA-Z0-9_-]*)*)$/.test(cli.tiapp.id)) {
				logger.error(`tiapp.xml contains an invalid app id "${cli.tiapp.id}"`);
			...
			if (!ti.validAppId(cli.tiapp.id)) {
				logger.error(`Invalid app id "${cli.tiapp.id}"`);
				logger.error('The app id must not contain Java reserved words.\n');
				process.exit(1);
			}
		}
		...
		if (!/^system|px|dp|dip|mm|cm|in$/.test(cli.tiapp.properties['ti.ui.defaultunit'].value)) {
			logger.error(`Invalid "ti.ui.defaultunit" property value "${cli.tiapp.properties['ti.ui.defaultunit'].value}"\n`);
			logger.log('Valid units:');
		...
		if (this.targetSDK) {
			...
			logger.warn(`${
				'<tool-api-level>'.cyan
			} has been deprecated, please specify the target SDK API using the ${
				'<uses-sdk>'.cyan
			} tag:`);
			logger.warn();
		...
		if (this.minSDK && this.realMinSDK < this.minSupportedApiLevel) {
			...
			} or newer, but is currently set to ${
				this.minSDK
			}${
				this.minSDK !== this.realMinSDK ? ` (${this.realMinSDK})` : ''
			}\n`);
			logger.log(
				appc.string.wrap(
					`Update the ${
						'android:minSdkVersion'.cyan
					} in the tiapp.xml or custom AndroidManifest to at least ${
						String(this.minSupportedApiLevel).cyan
					}:`,
		...
		if (this.targetSDK) {
			...
			if (this.realTargetSDK < this.minTargetApiLevel) {
				logger.error(`The target SDK API ${
					...
					this.targetSDK !== this.realTargetSDK ? ` (${this.realTargetSDK})` : ''
				} is not supported by Titanium SDK ${
					ti.manifest.version
				}`);
				logger.error(`The target SDK API version must be ${this.minTargetApiLevel} or newer\n`);
				logger.log(
					appc.string.wrap(
						`Update the ${
							'android:targetSdkVersion'.cyan
						} in the tiapp.xml or custom AndroidManifest to at least ${
							String(this.minTargetApiLevel).cyan
						}:`,
						config.get('cli.width', 100)
					)
				);
		...
		if (this.realTargetSDK < this.realMinSDK) {
			logger.error(`Target Android SDK API version must be ${this.minSDK} or newer\n`);
			process.exit(1);
		}

		if (this.realMaxSDK && this.realMaxSDK < this.realTargetSDK) {
			logger.error(`Maximum Android SDK API version must be greater than or equal to the target SDK API ${
				this.targetSDK
			}${
				this.targetSDK !== this.realTargetSDK ? ` (${this.realTargetSDK})` : ''
			}, but is currently set to ${
				...
				this.maxSDK !== this.realMaxSDK ? ` (${this.realMaxSDK})` : ''
			}\n`);
			process.exit(1);
		}
		...
		if (this.maxSupportedApiLevel && this.realTargetSDK > this.maxSupportedApiLevel) {
			...
			logger.warn(`Building with Android SDK API ${
				this.targetSDK
			}${
				this.targetSDK !== this.realTargetSDK ? ` (${this.realTargetSDK})` : ''
		...
		if (customABIs) {
			...
			this.abis.forEach(function (abi) {
				if (this.validABIs.indexOf(abi) === -1) {
					logger.error(`Invalid ABI "${abi}"\n`);
					logger.log('Valid ABIs:');
		...
		if (!this.buildOnly && /^device|emulator$/.test(this.target) && deviceId === undefined && config.get('android.autoSelectDevice', true)) {
			...
			function setDeviceId(device) {
				...
				if (device.googleApis) {
					gapi = (' (Google APIs supported)').grey;
				} else if (device.googleApis === null) {
					gapi = (' (Google APIs support unknown)').grey;
				}

				logger.info(`Auto selected ${
					cli.argv.target === 'device' ? 'device' : 'emulator'
				} ${device.name.cyan} ${device.version}${gapi}`);
			}
			...
			if (!deviceId) {
				logger.debug(`Searching for API >= ${apiLevel.cyan}`);
				for (let i = 0; i < len; i++) {
			...
			devices.forEach(function (device) {
				if (Array.isArray(device.abi) && !device.abi.some(function (a) { return this.abis.indexOf(a) !== -1; }.bind(this))) { // eslint-disable-line max-statements-per-line
					if (this.target === 'emulator') {
						logger.error(`The emulator "${
							device.name
						}" does not support the desired ABI${
							this.abis.length === 1 ? '' : 's'
						} ${`"${this.abis.join('", "')}"`}`);
					} else {
						logger.error(`The device "${
					...
					logger.error(`Supported ABIs: ${device.abi.join(', ')}\n`);

					logger.log('You need to add at least one of the device\'s supported ABIs to the tiapp.xml');
					logger.log();
					logger.log('<ti:app xmlns:ti="http://ti.tidev.io">'.grey);
		...
		if (this.debugPort || this.profilerPort) {
			...
			if (this.target === 'emulator') {
				...
				if (!emu) {
					logger.error(`Unable find emulator "${deviceId}"\n`);
					process.exit(1);
				} else if (!emu.sdcard) {
					logger.error(`The selected emulator "${emu.name}" does not have an SD card.`);
					if (this.profilerPort) {
						logger.error('An SD card is required for profiling.\n');
					} else {
						logger.error('An SD card is required for debugging.\n');
					}
					process.exit(1);
				}
			} else if (this.target === 'device' && deviceId === 'all' && this.devices.length > 1) {
				// fail, can't do 'all' for debug builds
				logger.error('Cannot debug application when --device-id is set to "all" and more than one device is connected.');
				logger.error('Please specify a single device to debug on.\n');
				process.exit(1);
			}
		}
		...
		if (fs.existsSync(buildDir)) {
			if (!afs.isDirWritable(buildDir)) {
				logger.error(`The build directory is not writeable: ${buildDir}\n`);
			...
			logger.error(`The project directory is not writeable: ${cli.argv['project-dir']}\n`);
		...
		return this.finalizeTiModules(modules);
	}

	async finalizeTiModules(modules) {
	...
	async initialize() {
		...
		if (!this.buildOnly && this.target === 'emulator') {
			...
			if (!emu) {
				// sanity check
				this.logger.error(`Unable to find Android emulator "${deviceId}"\n`);
				process.exit(0);
			}
			this.emulator = emu;
		}
	...
	loginfo() {
		this.logger.debug(`Titanium SDK Android directory: ${this.platformPath.cyan}`);
		this.logger.info(`Deploy type: ${this.deployType.cyan}`);
		this.logger.info(`Building for target: ${this.target.cyan}`);

		if (this.buildOnly) {
			this.logger.info('Performing build only');
		} else if (this.target === 'emulator') {
			this.logger.info(`Building for emulator: ${this.deviceId.cyan}`);
		} else if (this.target === 'device') {
			this.logger.info(`Building for device: ${this.deviceId.cyan}`);
		}

		this.logger.info(`Targeting Android SDK API: ${
			String(this.targetSDK + (this.targetSDK !== this.realTargetSDK ? ' (' + this.realTargetSDK + ')' : '')).cyan
		}`);
		this.logger.info(`Building for the following architectures: ${
			this.abis.join(', ').cyan
		}`);
		this.logger.info(`Signing with keystore: ${
			(this.keystore + ' (' + this.keystoreAlias.name + ')').cyan
		}`);

		this.logger.debug(`App ID: ${this.appid.cyan}`);
		this.logger.debug(`Classname: ${this.classname.cyan}`);

		if (this.allowDebugging && this.debugPort) {
			this.logger.info(`Debugging enabled via debug port: ${String(this.debugPort).cyan}`);
		} else {
			this.logger.info('Debugging disabled');
		}

		if (this.allowProfiling && this.profilerPort) {
			this.logger.info(`Profiler enabled via profiler port: ${String(this.profilerPort).cyan}`);
		} else {
	...
	checkIfShouldForceRebuild() {
		...
		if (this.cli.argv.force) {
			this.logger.info(`Forcing rebuild: ${'--force'.cyan} flag was set`);
			return true;
		}

		if (!fs.existsSync(this.buildManifestFile)) {
			this.logger.info(`Forcing rebuild: ${this.buildManifestFile.cyan} does not exist`);
			return true;
		}
		...
		if (this.target !== manifest.target) {
			...
			this.logger.info(`  Was: ${manifest.target}`);
			this.logger.info(`  Now: ${this.target}`);
			return true;
		}

		// check if the deploy type changed
		if (this.deployType !== manifest.deployType) {
			this.logger.info('Forcing rebuild: deploy type changed since last build');
		...
		if (this.classname !== manifest.classname) {
			this.logger.info('Forcing rebuild: classname changed since last build');
			this.logger.info(`  Was: ${manifest.classname}`);
			this.logger.info(`  Now: ${this.classname}`);
			return true;
		}
		...
		if (this.platformPath !== manifest.platformPath) {
			...
			this.logger.info(`  Now: ${this.platformPath}`);
			return true;
		}
		...
		if (!manifest.gitHash || manifest.gitHash !== ti.manifest.githash) {
			...
			this.logger.info(`  Was: ${manifest.gitHash}`);
			this.logger.info(`  Now: ${ti.manifest.githash}`);
			return true;
		}
		...
		if (this.modulesHash !== manifest.modulesHash) {
			...
			this.logger.info(`  Was: ${manifest.modulesHash}`);
			this.logger.info(`  Now: ${this.modulesHash}`);
			return true;
		}
		...
		if (this.tiapp.name !== manifest.name) {
			this.logger.info('Forcing rebuild: tiapp.xml project name changed since last build');
			this.logger.info(`  Was: ${manifest.name}`);
			this.logger.info(`  Now: ${this.tiapp.name}`);
			return true;
		}

		if (this.tiapp.id !== manifest.id) {
			this.logger.info('Forcing rebuild: tiapp.xml app id changed since last build');
			this.logger.info(`  Was: ${manifest.id}`);
			this.logger.info(`  Now: ${this.tiapp.id}`);
			return true;
		}
		...
		if (this.tiapp.publisher !== manifest.publisher) {
			...
			this.logger.info(`  Now: ${this.tiapp.publisher}`);
			return true;
		}

		if (this.tiapp.url !== manifest.url) {
			this.logger.info('Forcing rebuild: tiapp.xml url changed since last build');
			this.logger.info(`  Was: ${manifest.url}`);
			this.logger.info(`  Now: ${this.tiapp.url}`);
			return true;
		}

		if (this.tiapp.version !== manifest.version) {
			this.logger.info('Forcing rebuild: tiapp.xml version changed since last build');
			this.logger.info(`  Was: ${manifest.version}`);
			this.logger.info(`  Now: ${this.tiapp.version}`);
			return true;
		}

		if (this.tiapp.description !== manifest.description) {
			this.logger.info('Forcing rebuild: tiapp.xml description changed since last build');
			this.logger.info(`  Was: ${manifest.description}`);
			this.logger.info(`  Now: ${this.tiapp.description}`);
			return true;
		}

		if (this.tiapp.copyright !== manifest.copyright) {
			this.logger.info('Forcing rebuild: tiapp.xml copyright changed since last build');
			this.logger.info(`  Was: ${manifest.copyright}`);
			this.logger.info(`  Now: ${this.tiapp.copyright}`);
			return true;
		}

		if (this.tiapp.guid !== manifest.guid) {
			this.logger.info('Forcing rebuild: tiapp.xml guid changed since last build');
			this.logger.info(`  Was: ${manifest.guid}`);
			this.logger.info(`  Now: ${this.tiapp.guid}`);
			return true;
		}

		if (this.tiapp.icon !== manifest.icon) {
			this.logger.info('Forcing rebuild: tiapp.xml icon changed since last build');
			this.logger.info(`  Was: ${manifest.icon}`);
			this.logger.info(`  Now: ${this.tiapp.icon}`);
			return true;
		}

		if (this.tiapp.fullscreen !== manifest.fullscreen) {
			this.logger.info('Forcing rebuild: tiapp.xml fullscreen changed since last build');
			this.logger.info(`  Was: ${manifest.fullscreen}`);
			this.logger.info(`  Now: ${this.tiapp.fullscreen}`);
			return true;
		}

		if (this.tiapp['navbar-hidden'] !== manifest.navbarHidden) {
			this.logger.info('Forcing rebuild: tiapp.xml navbar-hidden changed since last build');
			this.logger.info(`  Was: ${manifest.navbarHidden}`);
			this.logger.info(`  Now: ${this.tiapp['navbar-hidden']}`);
			return true;
		}

		if (this.minSDK !== manifest.minSDK) {
			this.logger.info('Forcing rebuild: Android minimum SDK changed since last build');
			this.logger.info(`  Was: ${manifest.minSDK}`);
			this.logger.info(`  Now: ${this.minSDK}`);
			return true;
		}

		if (this.targetSDK !== manifest.targetSDK) {
			this.logger.info('Forcing rebuild: Android target SDK changed since last build');
			this.logger.info(`  Was: ${manifest.targetSDK}`);
			this.logger.info(`  Now: ${this.targetSDK}`);
			return true;
		}

		if (this.propertiesHash !== manifest.propertiesHash) {
			this.logger.info('Forcing rebuild: tiapp.xml properties changed since last build');
			this.logger.info(`  Was: ${manifest.propertiesHash}`);
			this.logger.info(`  Now: ${this.propertiesHash}`);
			return true;
		}
		...
		if (this.servicesHash !== manifest.servicesHash) {
			this.logger.info('Forcing rebuild: Android services in tiapp.xml SDK changed since last build');
			this.logger.info(`  Was: ${manifest.servicesHash}`);
			this.logger.info(`  Now: ${this.servicesHash}`);
			return true;
		}
	...
	async generateLibProjectForModule(moduleInfo) {
		// Validate arguments.
	...
	async gatherResources() {
		...
		// node_modules
		this.logger.info('Analyzing NPM package files');
		...
		// 		this.logger.error(`There is a project resource "${filename}" that conflicts with a native Android module`);
	 ...
	 * @returns {Promise<void>}
	 */
	async processDrawableFiles(files) {
		this.logger.debug('Copying Drawables');
	 ...
	 * @returns {Promise<void>}
	 */
	async processSplashesFiles(files) {
		this.logger.debug('Copying Splash Screens');
	 ...
	 * @returns {Promise<void>}
	 */
	copyUnmodifiedResources(resourcesToCopy) {
	 ...
	 */
	async ensureAppIcon(templateDir) {
	 ...
	 */
	async getNativeModuleBindings(jarFile) {
	...
	async generateJavaFiles() {
		...
		const copyTemplate = async (src, dest, ejsParams) => {
			this.logger.debug(`Copying template ${src.cyan} => ${dest.cyan}`);
		...
		if (android && android.activities) {
			...
			for (const activityName in android.activities) {
				...
				this.logger.debug(`Generating activity class: ${activity.classname.cyan}`);
		...
		if (android && android.services) {
			...
			for (const serviceName in android.services) {
				...
				if (service.type === 'interval') {
					...
					this.logger.debug(`Generating interval service class: ${service.classname.cyan}`);
				} else if (service.type === 'quicksettings') {
					tpl = quickSettingsServiceTemplate;
					this.logger.debug(`Generating quick settings service class: ${service.classname.cyan}`);
				} else {
					this.logger.debug(`Generating service class: ${service.classname.cyan}`);
				}
	...
	async generateI18N() {
		this.logger.info('Generating i18n files');
	...
	async generateSemanticColors() {
		this.logger.info('Generating semantic colors resources');
		const _t = this;
		...
		function writeXml(dom, dest, mode) {
			if (fs.existsSync(dest)) {
				_t.logger.debug(`Merging ${mode.cyan} semantic colors => ${dest.cyan}`);
			} else {
				_t.logger.debug(`Writing ${mode.cyan} semantic colors => ${dest.cyan}`);
			}
	...
	async generateAndroidManifest() {
		this.logger.info('Generating main "AndroidManifest.xml" files');
		...
		if (!this.config.get('android.mergeCustomAndroidManifest', true)) {
			this.logger.warn('Titanium CLI option "android.mergeCustomAndroidManifest" is no longer supported.');
			this.logger.warn('Use Google\'s "AndroidManifest.xml" feature "tools:remove" to remove XML elements instead.');
		}
	...
	async writeBuildManifest() {
		this.logger.info(`Writing build manifest: ${this.buildManifestFile.cyan}`);
