...
export class AndroidModuleBuilder extends Builder {
	...
	async migrate() {
		...
		if (!this.cli.argv.prompt) {
			if (isApiVersionUpdateRequired) {
				logger.error(`The module manifest apiversion is currently set to ${manifestModuleAPIVersion}`);
				logger.error(`Titanium SDK ${cliSDKVersion} Android module apiversion is at ${cliModuleAPIVersion}`);
				logger.error('Please update module manifest apiversion to match Titanium SDK module apiversion');
				logger.error(`and the minsdk to at least ${minSupportedSdkVersionString}`);
			} else {
				logger.error(`The module "manifest" file's minsdk is currently set to ${this.manifest.minsdk}`);
				logger.error(`Please update the file's minsdk to at least version ${minSupportedSdkVersionString}`);
			}
			process.exit(1);
		}
		...
		await new Promise((resolve, reject) => {
			let titleMessage;
			if (isApiVersionUpdateRequired) {
				titleMessage = `Detected Titanium ${
					cliSDKVersion
				} that requires API-level ${
					cliModuleAPIVersion
				}, but the module currently only supports ${
					manifestSDKVersion
				} and API-level ${
					manifestModuleAPIVersion
				}.`;
			} else {
				titleMessage = `Modules built with Titanium ${
					cliSDKVersion
				} cannot support Titanium versions older than ${
					minSupportedSdkVersionString
				}. The "manifest" file's minsdk must be updated.`;
			}
			fields.select({
				title: titleMessage,
				promptLabel: 'Do you want to migrate your module now?',
				default: 'yes',
				display: 'prompt',
				relistOnError: true,
				complete: true,
				...
				if (value !== 'yes') {
					logger.error('Please update the module\'s "manifest" file in order to build it.');
					process.exit(1);
				}
		...
		this.logger.info('Migrating module manifest...');

		// If a version is "1.0" instead of "1.0.0", semver currently fails. Work around it for now!
		if (!newVersion) {
			this.logger.warn(`Detected non-semantic version (${this.manifest.version}), will try to repair it!`);
			try {
				const semanticVersion = appc.version.format(this.manifest.version, 3, 3, true);
				newVersion = semver.inc(semanticVersion, 'major');
			} catch (err) {
				this.logger.error('Unable to migrate version for you. Please update it manually by using a semantic version like "1.0.0" and try the migration again.');
				process.exit(1);
			}
		}

		// Update the "apiversion" to the CLI API-version
		this.logger.info(`Setting ${'apiversion'.cyan} to ${cliModuleAPIVersion.cyan}`);
		this.manifest.apiversion = cliModuleAPIVersion;

		// Update the "minsdk" to the required CLI SDK-version
		this.logger.info(`Setting ${'minsdk'.cyan} to ${minSupportedSdkVersionString.cyan}`);
		this.manifest.minsdk = minSupportedSdkVersionString;

		// Update the "apiversion" to the next major
		this.logger.info(`Bumping version from ${this.manifest.version.cyan} to ${newVersion.cyan}`);
	...
	async validate(logger, config, cli) {
		...
		console.log('validate after config');
	...
	async dirWalker(directoryPath, callback) {
	...
	async loginfo() {
		this.logger.info(`Assets Dir: ${this.assetsDir.cyan}`);
		this.logger.info(`Documentation Dir: ${this.documentationDir.cyan}`);
		this.logger.info(`Example Dir: ${this.exampleDir.cyan}`);
		this.logger.info(`Platform Dir: ${this.platformDir.cyan}`);
		this.logger.info(`Resources Dir: ${this.resourcesDir.cyan}`);
	}
		...
		if (await fs.exists(this.libsDir)) {
			...
			await this.dirWalker(this.libsDir, (filePath, fileName) => {
				if (fileName === MODULE_LIB_FILE_NAME) {
					this.logger.debug(`Removing ${filePath.cyan}`);
	...
	async updateModuleFiles() {
		...
		if (hasJSFile && !this.manifest.commonjs) {
			...
			this.logger.info('Manifest re-written to set commonjs value');
		}
	}
	...
	async packageZip() {
		this.logger.info('Packaging the module');
		...
		*/
		...
		this.logger.info(`Writing module zip: ${moduleZipPath}`);
		dest.finalize();
	}
	...
	async runModule(cli) {
		...
		this.logger.debug(`Created example project ${tmpProjectDir.cyan}`);
