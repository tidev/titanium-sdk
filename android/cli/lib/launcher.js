/*
 * launcher.js: Titanium Android launcher service
 *
...
export function createAndroidLauncher({ logger, config, cli }) {
	let deviceInfo = [];
	const ignoreLog = config.cli.ignoreLog || [];

	function preCompile(builder, finished) {
	...
	function postCompile(builder, finished) {
			...
			function (next) {
				...
				async.eachSeries(deviceInfo, function (device, cb) {
					...
					(function startApp() {
						...
						adb.startApp(device.id, builder.appid, builder.classname + 'Activity', function (_err) {
							if (watchingPid) {
								return;
							}
							watchingPid = true;
