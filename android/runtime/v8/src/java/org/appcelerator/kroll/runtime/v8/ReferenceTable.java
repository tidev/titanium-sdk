...
public final class ReferenceTable
{
   ...
   * created. FIXME Handle "wrapping" the value around to Long.MIN_VALUE when
   * Long.MAX_VALUE is reached? What if we ever reach back up to 0 - we need to
   * skip it!
   */
   ...
   */
	...
	/**
   * Destroy the reference in our map. Done when we're deleting the native
   * proxy.
   *
   ...
   */
	public static void destroyReference(long key)
	{
		...
		// If it's an V8Object, set the ptr to 0, because the proxy is dead on C++
		// side This *should* prevent the native code from trying to reconstruct the
		// proxy for any reason
	...
	/**
   * Makes the reference "weak" which allows it to be collected if no other
   * references remain.
   *
   ...
   */
	...
	/**
   * Makes the reference "soft" which which are cleared at the discretion of the
   * garbage collector.
   *
   ...
   */
	...
	/**
   * Make the reference strong again to prevent future collection. This method
   * will return null if the reference was invalidated and the object collected.
   *
